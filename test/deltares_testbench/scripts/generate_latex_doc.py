...
def generate_pdf(u_dir, u_doc):
    ...
    if os.path.exists(u_dir):
        ...
        print_stderr("##teamcity[testStarted  name='Generating: %s' message='Nothing to build' captureStandardOutput='true']" % (u_doc))
        print_stderr("##teamcity[testFailed name='Generating: %s' message='Directory does not exists' details='%s']" % ( u_doc, u_dir))
    ...
    if svn_build == '':
        ...
        if _draft:
            ...
            print_stderr("##teamcity[testStarted  name='Generating: %s' message='Draft' captureStandardOutput='true']" % (u_doc))
        else:
            print_stderr("##teamcity[testStarted  name='Generating: %s' message='Final' captureStandardOutput='true']" % (u_doc))

        to_execute = '%s update' % _svnexe
        svn_update = subprocess.Popen(to_execute, stdout=subprocess.PIPE, stderr=subprocess.STDOUT).wait()
        if svn_update == '':
            print_stderr("##teamcity[testFailed name='Generating: %s' message='SVN Update Failed' details='%s']" % (u_doc, to_execute))
            _build_failure += 1
            return 1

        error = run_pdflatex(u_doc)
        if error == 1:
            print_stderr("##teamcity[testFailed name='Generating: %s' message='PDFLaTeX Run 1 Failed: %s' details='%s']" % (u_doc, os.getcwd(), _pdflatex))
            print_stderr("##teamcity[testFinished name='Generating: %s']" % (u_doc))
            _build_failure += 1
            return 1
        error = run_bibtex(u_doc)
        if error == 1:
            print_stderr("##teamcity[testFailed name='Generating: %s' message='BIBtex Failed: %s' details='%s']" % (u_doc, os.getcwd(), _bibtex))
            _build_failure += 1
            return 1
        error = run_pdflatex(u_doc)
        if error == 1:
            print_stderr("##teamcity[testFailed name='Generating: %s' message='PDFLaTeX Run 2 Failed: %s' details='%s']" % (u_doc, os.getcwd(), _pdflatex))
            _build_failure += 1
            return 1
        if os.path.isfile(u_doc + '.idx'):
            error = run_make_index(u_doc + '.idx')
            if error == 1:
                print_stderr("##teamcity[testFailed name='Generating: %s' message='MakeIndex Failed: %s' details='%s']" % (u_doc, os.getcwd(), _pdflatex))
                _build_failure += 1
                return 1
        error = run_pdflatex(u_doc)
        if error == 1:
            print_stderr("##teamcity[testFailed name='Generating: %s' message='PDFLaTeX Run 3 Failed: %s' details='%s']" % (u_doc, os.getcwd(), _pdflatex))
            _build_failure += 1
            return 1
        error = run_pdflatex(u_doc)
        if error == 1:
            print_stderr("##teamcity[testFailed name='Generating: %s' message='PDFLaTeX Run 4 Failed' details='%s']" % (u_doc, _pdflatex))
            _build_failure += 1
            return 1
        print_stderr("##teamcity[testFinished name='Generating: %s']" % (u_doc))
        _build += 1
