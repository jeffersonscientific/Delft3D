...
docker run --rm \
           ...
           "$container" python3.11 -m tools.h7.upload_folder_to_minio devops-teamcity-artifacts

docker run --rm \
           -v=$HOME/.aws:/root/.aws:ro \
           "$container" python3.11 tools/h7/start_tc_build.py "$teamcity_config" container_id:"$container_id" "$tc_account" "$tc_password"
