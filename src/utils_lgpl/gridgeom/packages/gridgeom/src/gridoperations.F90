...
contains
   ...
   subroutine SAVENET()
      ...
      do K = 1, KX
         ...
         if (LS >= 1) then
            ...
            if (LS0 < LS) then
               if (LS0 >= 1 .and. allocated(NOD0(K)%LIN)) deallocate (NOD0(K)%LIN)
   ...
   logical function alreadycell(N, K, L)
      ...
      do i = 1, N
         ...
         do j = 1, netcell(icell(i))%N
            ...
            if (lnn(LL) /= 2) cycle ! this also excludes all members of the link set
            ...
            dum = 0
            where (icell == kL) dum = 1
            if (sum(dum) == 0) cycle ! kL not a member
            dum = 0
            where (icell == kR) dum = 1
            if (sum(dum) == 0) cycle ! kR not a member

            !        check if the link connects nodes in the node set
            dum = 0
            where (K == kn(1, LL)) dum = 1
            if (sum(dum) == 0) cycle ! first node of link not a member
            dum = 0
            where (K == kn(2, LL)) dum = 1
            if (sum(dum) == 0) cycle ! second node of link not a member
      ...
      do i = 1, N
         ...
         if (kcom == 0) then ! set common node
            kcom = knod
         else if (knod /= kcom) then ! check with common node
            kcom = 0
            exit
         end if
      end do
   ...
   subroutine OTHERNODECHK(K1, L1, K2)
      ...
      if (KN(3, L1) /= 2 .and. KN(3, L1) /= 0) return
...
100   continue
   ...
   subroutine findcells(jp)
      ...
      if (JP_ == 0) then
         ...
         call FINDTRIS(0)
         if (jafold == 1) call FINDTRIS(1)
      else if (JP_ == 4) then
         call FINDQUADS(0)
         if (jafold == 1) call FINDQUADS(1)
      else if (JP_ == 5) then
         call FINDPENTAS(0)
         if (jafold == 1) call FINDPENTAS(1)
      else if (JP_ == 6) then
         call FINDHEXAS(0)
         if (jafold == 1) call FINDHEXAS(1)
      else if (JP_ == 11) then
         call FINDPENTAS(0)
         call FINDHEXAS(0)
         if (jafold == 1) then
      ...
      do K1 = 1, NUMK
         ...
         if (KC(K1) == 1) then
            ...
            kklp: do KK = 1, NMK(K1)
               ...
               kkk = 1
               do while (nod(k2)%lin(kkk) /= L)
                  kkk = kkk + 1
               end do
               do KKK_ = 1, min(NMK(K2), nmkmax)
                  kkk = kkk - 1
                  if (kkk < 1) kkk = kkk + nmk(k2)
                  if (kkk > nmk(k2)) kkk = kkk - nmk(k2)

                  LL = NOD(K2)%LIN(KKK); if (LL == L) cycle
                  if (LNN(LL) >= 2) cycle
                  ...
                  if (K3 /= K1) then
                     ...
                     kkkk = 1
                     do while (nod(k3)%lin(kkkk) /= LL)
                        kkkk = kkkk + 1
                     end do
                     do KKKK_ = 1, min(NMK(K3), nmkmax)
                        kkkk = kkkk - 1
                        if (kkkk < 1) kkkk = kkkk + nmk(k3)
                        if (kkkk > nmk(k3)) kkkk = kkkk - nmk(k3)

                        LLL = NOD(K3)%LIN(KKKK); if (LLL == LL .or. LLL == L) cycle
                        if (LNN(LLL) >= 2) cycle
                        call OTHERNODECHK(K3, LLL, K4); if (K4 == 0) cycle
                        if (rechtsaf(K2, K3, K4)) cycle
                        if (K4 == K1) then ! TRI GEVONDEN
                           ...
                           if (lnn(L) > 0 .and. lnn(LL) > 0 .and. lnn(LLL) > 0) then
                              if (lne(1, L) == lne(1, LL) .and. lne(1, L) == lne(1, LLL)) then
                                 cycle
                  ...
                  if (K3 /= K1) then
                     ...
                     do KKKK_ = 1, min(NMK(K3), nmkmax)
                        ...
                        if (K4 /= K2) then
                           ...
                           kkkkk = 1
                           do while (nod(k4)%lin(kkkkk) /= LLL)
                              kkkkk = kkkkk + 1
                           end do
                           do KKKKK_ = 1, min(NMK(K4), nmkmax)
                              kkkkk = kkkkk - 1
                              if (kkkkk < 1) kkkkk = kkkkk + nmk(k4)
                              if (kkkkk > nmk(k4)) kkkkk = kkkkk - nmk(k4)

                              LLLL = NOD(K4)%LIN(KKKKK)
                              if (LLLL == LLL .or. LLLL == LL .or. LLLL == L) cycle
                              if (LNN(LLLL) >= 2) cycle
                              call OTHERNODECHK(K4, LLLL, K5); if (K5 == 0) cycle
                              if (rechtsaf(K3, K4, K5)) cycle
                              if (K5 == K1) then ! PANEEL GEVONDEN
                                 if (LNN(L) > 1 .or. LNN(LL) > 1 .or. LNN(LLL) > 1 .or. LNN(LLLL) > 1) exit

                                 !  SPvdP: check and see if cell already exist
                                 if (lnn(L) > 0 .and. lnn(LL) > 0 .and. lnn(LLL) > 0 .and. lnn(LLLL) > 0) then
                                    if (lne(1, L) == lne(1, LL) .and. lne(1, L) == lne(1, LLL) .and. lne(1, L) == lne(1, LLLL)) then
                                       cycle
                                    else ! more expensive check
                                       ...
                                       !                               do not allow folded cells when all links already have neighboring cells
                                       if (kkk_ /= 1 .or. kkkk_ /= 1 .or. kkkkk_ /= 1) then
                                          cycle
                                       end if
                                    end if
                                 end if
               ...
               do KKK_ = 1, min(NMK(K2), nmkmax)
                  ...
                  if (K3 /= K1) then
                     ...
                     do KKKK_ = 1, min(NMK(K3), nmkmax)
                        ...
                        LLL = NOD(K3)%LIN(KKKK)
                        if (LLL == LL .or. LLL == L) cycle
                        if (LNN(LLL) >= 2) cycle
                        call OTHERNODECHK(K3, LLL, K4); if (K4 == 0) cycle
                        if (rechtsaf(K2, K3, K4)) cycle
                        if (K4 /= K2 .and. K4 /= K1) then
                           ...
                           do KKKKK_ = 1, min(NMK(K4), nmkmax)
                              ...
                              call OTHERNODECHK(K4, LLLL, K5); if (K5 == 0) cycle
                              if (rechtsaf(K3, K4, K5)) cycle
                              if (K5 /= K3 .and. K5 /= K2 .and. K5 /= K1) then

                                 kkkkkk = 1
                                 do while (nod(k5)%lin(kkkkkk) /= LLLL)
                                    kkkkkk = kkkkkk + 1
                                 end do
                                 do KKKKKK_ = 1, min(NMK(K5), nmkmax)
                                    kkkkkk = kkkkkk - 1
                                    if (kkkkkk < 1) kkkkkk = kkkkkk + nmk(k5)
                                    if (kkkkkk > nmk(k5)) kkkkkk = kkkkkk - nmk(k5)

                                    LLLLL = NOD(K5)%LIN(KKKKKK)
                                    if (LLLLL == LLLL .or. LLLLL == LLL .or. LLLLL == LL .or. LLLLL == L) cycle
                                    if (LNN(LLLLL) >= 2) cycle
                                    call OTHERNODECHK(K5, LLLLL, K6); if (K6 == 0) cycle
                                    if (rechtsaf(K4, K5, K6)) cycle
                                    if (K6 == K1) then ! PENTA GEVONDEN
                                       if (LNN(L) > 1 .or. LNN(LL) > 1 .or. LNN(LLL) > 1 .or. &
                                           LNN(LLLL) > 1 .or. LNN(LLLLL) > 1) exit

                                       !  SPvdP: check and see if cell already exist
                                       if (lnn(L) > 0 .and. lnn(LL) > 0 .and. lnn(LLL) > 0 .and. &
                                           lnn(LLLL) > 0 .and. lnn(LLLLL) > 0) then
                                          if (lne(1, L) == lne(1, LL) .and. lne(1, L) == lne(1, LLL) .and. &
                                              lne(1, L) == lne(1, LLLL) .and. lne(1, L) == lne(1, LLLLL)) then
                                             cycle
                                          else ! more expensive check
                                             ...
                                             !                                  do not allow folded cells when all links already have neighboring cells
                                             if (kkk_ /= 1 .or. kkkk_ /= 1 .or. kkkkk_ /= 1 .or. kkkkkk_ /= 1) then
                                                cycle
                                             end if
                                          end if
                                       end if
                           ...
                           do KKKKK_ = 1, min(NMK(K4), nmkmax)
                              ...
                              if (K5 /= K3 .and. K5 /= K2 .and. K5 /= K1) then
                                 ...
                                 do KKKKKK_ = 1, min(NMK(K5), nmkmax)
                                    ...
                                    call OTHERNODECHK(K5, LLLLL, K6); if (K6 == 0) cycle
                                    if (rechtsaf(K4, K5, K6)) cycle
                                    if (K6 /= K4 .and. K6 /= K3 .and. K6 /= K2 .and. K6 /= K1) then

                                       kkkkkkk = 1
                                       do while (nod(k6)%lin(kkkkkkk) /= LLLLL)
                                          kkkkkkk = kkkkkkk + 1
                                       end do
                                       do KKKKKKK_ = 1, min(NMK(K6), nmkmax)
                                          kkkkkkk = kkkkkkk - 1
                                          if (kkkkkkk < 1) kkkkkkk = kkkkkkk + nmk(k6)
                                          if (kkkkkkk > nmk(k6)) kkkkkkk = kkkkkkk - nmk(k6)

                                          LLLLLL = NOD(K6)%LIN(KKKKKKK)
                                          if (LLLLLL == LLLLL .or. LLLLLL == LLLL .or. &
                                              LLLLLL == LLL .or. LLLLLL == LL .or. LLLLLL == L) cycle
                                          if (LNN(LLLLLL) >= 2) cycle
                                          call OTHERNODECHK(K6, LLLLLL, K7); if (K7 == 0) cycle
                                          if (rechtsaf(K5, K6, K7)) cycle
                                          if (K7 == K1) then ! HEXA GEVONDEN
                                             if (LNN(L) > 1 .or. LNN(LL) > 1 .or. LNN(LLL) > 1 .or. &
                                                 LNN(LLLL) > 1 .or. LNN(LLLLL) > 1 .or. LNN(LLLLLL) > 1) exit

                                             !  SPvdP: check and see if cell already exist
                                             if (lnn(L) > 0 .and. lnn(LL) > 0 .and. lnn(LLL) > 0 .and. &
                                                 lnn(LLLL) > 0 .and. lnn(LLLLL) > 0 .and. lnn(LLLLLL) > 0) then
                                                if (lne(1, L) == lne(1, LL) .and. lne(1, L) == lne(1, LLL) .and. &
                                                    lne(1, L) == lne(1, LLLL) .and. lne(1, L) == lne(1, LLLLL) .and. lne(1, L) == lne(1, LLLLLL)) then
                                                   cycle
                                                else ! more expensive check
                                                   ...
                                                   !                                       do not allow folded cells when all links already have neighboring cells
                                                   if (kkk_ /= 1 .or. kkkk_ /= 1 .or. kkkkk_ /= 1 .or. kkkkkk_ /= 1 .or. kkkkkkk_ /= 1) then
                                                      cycle
                                                   end if
                                                end if
                                             end if
   ...
   elemental recursive subroutine INCELLS(XA, YA, KIN)
      ...
      do K = 1, NUMP
         ...
         !Method: https://wrfranklin.org/Research/Short_Notes/pnpoly.html. Copyright (c) 1970-2003, Wm. Randolph Franklin
         in = .false.
         j = NN 
         do i = 1, NN
            if ((yh(i) > ya) .neqv. (yh(j) > ya)) then
               if (xa < (xh(j) - xh(i)) * (ya - yh(i)) / (yh(j) - yh(i)) + xh(i)) then
                  in = .not. in
               end if
            end if
            j=i
   ...
   subroutine get_cellpolygon(n, Msize, nn, rcel, xv, yv, LnnL, Lorg, zz)
      ...
      do m = 1, nn
         ...
         mp1 = m + 1; if (mp1 > nn) mp1 = mp1 - nn
         k1 = netcell(n)%nod(m)
         k2 = netcell(n)%nod(mp1)

         if (kpole(m) == 1 .and. kpole(mp1) /= 1) then
            num = num + 1
            xv(num) = xk(k2)
            yv(num) = yk(k1)
            if (zk(k1) /= DMISS) then
            ...
            lnnl(num) = LNN(netcell(n)%lin(m))
            Lorg(num) = m
         else if (kpole(m) /= 1 .and. kpole(mp1) == 1) then
            num = num + 1
            xv(num) = xk(k1)
            yv(num) = yk(k1)
            if (zk(k1) /= DMISS) then
            ...
            mp2 = mp1 + 1; if (mp2 > nn) mp2 = mp2 - nn
            if (kpole(mp2) == 1) then
               Lorg(num) = mp1
            else ! add dummy link
               Lorg(num) = 0
      ...
      if (jsferic == 1) then
         do m = 1, nn
            mp1 = m + 1; if (mp1 > nn) mp1 = mp1 - nn
            if (xv(mp1) - xv(m) > 180d0) then
               xv(mp1) = xv(mp1) - 360d0
            else if (xv(mp1) - xv(m) < -180d0) then
               xv(mp1) = xv(mp1) + 360d0
            end if
      ...
      do L = 1, numl
         if (kn(3, L) == 1 .or. kn(3, L) == 6) then !  .or. kn(3,L) == 4) THEN
            ...
            if (validOneDMask) then
               ...
               if (oneDMask(k2ClientIndex) == 0) then
                  dis2 = DISMIN
               end if
   ...
   subroutine getcellweightedcenter(n, xz, yz, zz)
      ...
      if (circumormasscenter /= 1d0) then
...
999   continue
   ...
   subroutine make_dual_cell(k, N, rcel, xx, yy, num, Wu1Duni)
      ...
      do i = 1, NN
         ...
         if (kn(3, L) /= 2 .and. kn(3, L) /= 0) ja2D = 0
      end do
...
!           fix for periodic, spherical coordinates
            if (jsferic == 1) then
               if (xk(k2) - xk(k1) > 180d0) then
                  xx(num) = xx(num) - 180d0
               else if (xk(k2) - xk(k1) < -180d0) then
                  xx(num) = xx(num) + 180d0
               end if
            end if
...
!        fix for periodic, spherical coordinates
         if (xc - xk(k) > 180d0) then
            xc = xc - 360d0
         else if (xc - xk(k) < -180d0) then
            xc = xc + 360d0
         end if
...
!          find own link index
            ii = 1
            do while (netcell(ic1)%lin(ii) /= L .and. ii < netcell(ic1)%N)
               ii = ii + 1
            end do
!          check if previous netlink in netcell ic1 is netlink (i+1)
            iim1 = ii - 1; if (iim1 < 1) iim1 = iim1 + netcell(ic1)%N

            if (netcell(ic1)%lin(iim1) == Lp1) then
               ja = 1
            end if
         end if
...
!----------------------------------------------------------------------!
   ...
   function ggeo_get_links(arrayfrom, arrayto, linkType, start_index) result(ierr)
      ...
      if (ierr /= 0 .or. (.not. (allocated(mesh1dInternalToClientMapping))) .or. (.not. (allocated(mesh2dMapping)))) then
         ierr = -1
         return
      end if
   ...
   function ggeo_connect_edges(startEdge, endEdge, connectionMask, numedege, edgenodes) result(ierr)
      ...
      if ((connectionMask(startEdge) == 0) .or. (connectionMask(endEdge) == 0)) then
         numedege = numedege + 1
   ...
   function ggeo_count_or_create_edge_nodes(branchidx, branchoffset, sourcenodeid, targetnodeid, branchlength, startindex, numedege, edgenodes) result(ierr) !edge_nodes
      ...
      connectionMask = 0
      do br = 1, nbranches
         if ((meshnodemapping(1, br) == -1) .or. (meshnodemapping(2, br) == -1)) then
            ierr = -1
            return
         end if
         ...
         do n = 1, nmeshnodes
            if (correctedBranchidx(n) == br .and. (n /= st) .and. (n /= en)) then
               kk = kk + 1
   ...
   function ggeo_make1D2Dembeddedlinks(jsferic, jasfer3D, oneDmask) result(ierr)
      ...
      do l = 1, numl1d
         ...
         if (isInCell(k1) == -1) then
            call incells(xk(k1), yk(k1), cellId)
            isInCell(k1) = cellId
         end if
         if (isInCell(k2) == -1) then
         ...
         !no cells found
         if (nCellsInSearchRadius == 0) cycle
         !reallocate if necessary
         call realloc_results_kdtree(treeinst, nCellsInSearchRadius)
         !find nearest cells
         call kdtree2_n_nearest(treeinst%tree, treeinst%qv, nCellsInSearchRadius, treeinst%results)
         ...
         do k = 1, nCellsInSearchRadius
            ...
            do kk = 1, numberCellNetlinks
               ...
               if (isCrossing == 1) then
                  ...
                  if (ldistance <= rdistance .and. isInCell(k1) >= 1) then
                  ...
                  else if (ldistance > rdistance .and. isInCell(k2) >= 1) then
                     if (validOneDMask) then !again, Fortran does not have logical and two nested if statement are needed
                        k2ClientIndex = mesh1dInternalToClientMapping(k2)
                        if (oneDmask(k2ClientIndex) == 1) then
                           !call setnewpoint(xz(cellId),yz(cellId),zk(cellId), newPointIndex)
