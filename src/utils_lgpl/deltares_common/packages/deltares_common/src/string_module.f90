...
contains
   ...
   pure subroutine get_version_major_minor_integer(version_string, major, minor, success)
      ...
      integer :: idot, iend, ierr
      logical :: isnum

      success = .false.

      idot = index(version_string, '.')
      if (idot == 0) then
         return
      end if

      read (version_string(1:idot - 1), *, iostat=ierr) major
      if (ierr /= 0) then
         return
      end if
      ...
      isnum = .true.
      do while (isnum)
         if (iend + 1 > len(version_string)) then
            isnum = .false.
         elseif (scan(version_string(iend + 1:iend + 1), '0123456789') /= 0) then
            iend = iend + 1
         else
            isnum = .false.
         end if
      ...
      read (version_string(idot + 1:iend), *, iostat=ierr) minor
      if (ierr /= 0) then
         return
      end if
