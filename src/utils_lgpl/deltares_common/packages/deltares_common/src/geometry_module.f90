...
10       continue
         ...
         if (XL >= min(X1, X2) .and. XL <= max(X1, X2)) then
            ...
            if (XL == X1 .and. YL == Y1 .or. &
                !              op punt 1
                (X1 == X2 .and. YL >= min(Y1, Y2) .and. YL <= max(Y1, Y2)) .or. &
                !              op verticale lijn
...
!     deallocate
   ...
   subroutine GAUSSJ(A, N, NP, B, M, MP)
      ...
      do I = 1, N
         ...
         INDXR(I) = IROW
         INDXC(I) = ICOL
         if (A(ICOL, ICOL) == 0.) write (*, *) 'Singular matrix'
...
!     check if circumcenter is inside cell
   ...
   subroutine GETCIRCUMCENTER(nn, xv, yv, lnnl, xz, yz, jsferic, jasfer3D, jglobe, jins, dmiss, dxymis, dcenterinside, circumcenter_method_dummy)
      ...
      if (nn == 3 .and. jglobe == 0) then ! for triangles
         ...
         if (nintlinks > 1 .or. nn == 3) then
            ! the computation will start if any of the following conditions hold:
            ! nintlinks > 1 (there are more than one netlink with two neighbouring grid cells);
            ! nn == 3 (triangle).
            do k = 1, 100 ! Zhang, Schmidt and Perot 2002, formula A3
               xccfo = xccf
               yccfo = yccf
               do m = 1, nn
                  if (lnnl(m) == 2 .or. nn == 3 .or. circumcenter_method_dummy == ALL_NETLINKS_LOOP) then
                     ! A netlink will be considered if any of the following conditions hold:
                     ! lnnl(m) == 2 (internal links, net link with two neighbouring cells);
                     ! nn == 3 (triangle);
                     ! circumcenter_method_dummy == ALL_NETLINKS_LOOP (consider all net links, also those on the outline of the grid, iteration per loop)
                     xe1 = xv(m)
                     ye1 = yv(m)
                     m2 = m + 1; if (m == nn) m2 = 1
                     xe2 = xv(m2)
                     ...
                     if (circumcenter_method_dummy == INTERNAL_NETLINKS_EDGE) then
                        ...
                        ! (consider iteration per loop)
                        xcc3 = getdx(xe3, ye3, xccfo, yccfo, jsferic)
                        ycc3 = getdy(xe3, ye3, xccfo, yccfo, jsferic)
                     end if
