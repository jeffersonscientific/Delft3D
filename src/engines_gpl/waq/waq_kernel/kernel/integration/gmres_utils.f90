...
module m_gmres_utils
    ...
    public :: fill_matrix_hz_upwind_diff_vl_upwind_diff, fill_matrix_hz_backward_diff_vl_central_diff, &
            ...
            fill_matrix_for_theta_algorithm, fill_matrix_central_difference_space, fill_rhs_initial_guess_theta, &
...
contains
    ...
    subroutine fill_matrix_central_difference_space(disp, disper, area, flow, aleng, &
            velo, bound, ipoint, num_substances_total, substance_i, &
        ...
        integer(kind = int_wp), intent(in) :: substance_i           !< Index of system currently considered
        ...
        if (timon) call timstrt ("fill_matrix_central_difference_space", ithandl)
        ...
        do iq = 1, num_exchanges
            ...
            e = e * dl
            if (idpnt(substance_i)>0) e = e + disper((iq - 1) * num_dispersion_arrays + idpnt(substance_i)) * dl
            if (ivpnt(substance_i)>0) q = q + velo  ((iq - 1) * num_velocity_arrays + ivpnt(substance_i)) * a
            q1 = f1 * q
            q2 = f2 * q
            if (i < 0) goto 10
            if (j < 0) goto 30
            ...
            i4 = (j - 1) * nsys + 1
            do i3 = substance_i, substance_i + nsys - 1
        ...
        if (timon) call timstop(ithandl)
    end subroutine fill_matrix_central_difference_space
