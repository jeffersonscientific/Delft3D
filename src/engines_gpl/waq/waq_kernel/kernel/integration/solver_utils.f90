...
module m_solver_utils
    ...
    private
    public :: solve_linear_equations_lu_decomposition, prepare_matrix_for_solver
...
contains
    ...
    subroutine solve_linear_equations_lu_decomposition(n, nuc, nlc, m, a, b, option)
        ...
        if (timon) call timstrt ("solve_linear_equations_lu_decomposition", ithandl)
        nmuc = n - nuc
        nmlc = n - nlc
        ndm1 = nuc + nlc
        nd = ndm1 + 1
        ...
        if (abs(p) < 1.0e-35) then
            write(6, '('' Matrix for solve_linear_equations_lu_decomposition singular at element:'',I5)') &
                    k1 / nd + 1
    ...
    end subroutine solve_linear_equations_lu_decomposition

    !> Zeroes the matrix, updates first-order term on the diagonal and compresses DERIV for use in
    ! solve_linear_equations_lu_decomposition
    subroutine prepare_matrix_for_solver(conc, deriv, amass, amat, num_cells, num_substances_total, substance_i, &
            num_substances_rhs, num_codiagonals)
        ...
        integer(kind = int_wp), intent(in) :: substance_i   !< System considered
        integer(kind = int_wp), intent(in) :: num_substances_rhs   !< Number of systems to take
        integer(kind = int_wp), intent(in) :: num_codiagonals !< Number of codiagonals


        ! Local variables
        integer(kind = int_wp) :: ntot, i, istep, iset, cell_i, ioff
        integer(kind = int_wp) :: ithandl = 0

        if (timon) call timstrt ("prepare_matrix_for_solver", ithandl)
        ...
        do cell_i = 1, num_cells
            amat(iset) = -conc(substance_i, cell_i) + amass(cell_i)
            iset = iset + istep
        end do
        ...
        ioff = 0
        do cell_i = 1, num_cells
            do i = substance_i, substance_i + num_substances_rhs - 1
    ...
    end subroutine prepare_matrix_for_solver
end module m_solver_utils
