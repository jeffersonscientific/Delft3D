...
!     kind           function         name                   description
      ...
      real(kind=sp), intent(inout)  ::deriv (notot,noseg)  ! explicit derivative in mass/s
...
!     Local variables     :
      ...
      integer(kind=int_32) ::ifrom_1, ito_1  ! volume numbers one further at both sides
      ...
      real(kind=sp) ::f1, f2          ! interpolation factor central differences
      real(kind=sp) ::g1, g2          ! interpolation factor boundaries
      real(kind=sp) ::de, dv          ! help variables Lax Wendroff scheme
      ...
      real(kind=sp) ::c               ! velocity Courant number
      real(kind=sp) ::d2              ! diffusion courant number
      real(kind=sp) ::su              ! upwind half length
