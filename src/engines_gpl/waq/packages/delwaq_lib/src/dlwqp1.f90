...
contains
   ...
   subroutine dlwqp1(lun, lchar, &
      ...
      call getcom('-cs', 0, lfound, idummy, rdummy, cdummy, ierr2)
      if (lfound) then
         call setzmo(1)
      end if
      ...
      call getcom('-m', 1, lfound, mlevel, rdummy, cdummy, ierr2)
      if (lfound) then
         if (ierr2 .eq. 0) then
            call setmmo(mlevel)
         else
      ...
      call getcom('-a', 1, lfound, idummy, rdummy, cdummy, ierr2)
      if (lfound) then
      ...
      call getcom('-np', 0, lfound, idummy, rdummy, cdummy, ierr2)
      if (lfound) then
         swi_nopro = .true.
      ...
      if (.not. swi_nopro) then
         call getcom('-p', 3, lfound, idummy, rdummy, pdffil, ierr2)
         if (lfound) then
            if (ierr2 .ne. 0) then
               pdffil = ' '
            end if
      ...
      call getcom('-eco', 3, lfound, idummy, rdummy, blmfil, ierr2)
      if (lfound) then
         l_eco = .true.
         line = ' '
         call monsys(line, 1)
