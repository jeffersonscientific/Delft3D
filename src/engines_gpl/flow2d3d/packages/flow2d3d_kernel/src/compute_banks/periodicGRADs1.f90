subroutine periodicGRADs1(dsduuCENTR,dsdvvCENTR,nlb,mlb,nub,mub, gdp)  
!----- GPL ---------------------------------------------------------------------
!                                                                               
...
!!--description-----------------------------------------------------------------
!
!   Function:   periodic dsduuCENTR and dsdvvCENTR  at periodic boundaries 
!               
...
! 
  ...
  real(fp), dimension(nlb:nub,mlb:mub)  , intent(inout)               :: dsduuCENTR      
  real(fp), dimension(nlb:nub,mlb:mub)  , intent(inout)               :: dsdvvCENTR     
!
! local variables
!
  integer                    :: k 
...
!
    ...
    mPH_ext => gdp%gdimbound%mPH_ext
      ...
      do k=1,nrPER  
         dsduuCENTR(nPQ_ext(k),mPQ_ext(k))   = dsduuCENTR(nPH_int(k),mPH_int(k))  
         dsduuCENTR(nPH_ext(k),mPH_ext(k))   = dsduuCENTR(nPQ_int(k),mPQ_int(k))  
         dsdvvCENTR(nPQ_ext(k),mPQ_ext(k))   = dsdvvCENTR(nPH_int(k),mPH_int(k))  
         dsdvvCENTR(nPH_ext(k),mPH_ext(k))   = dsdvvCENTR(nPQ_int(k),mPQ_int(k))  
      enddo
!
RETURN
end subroutine periodicGRADs1
