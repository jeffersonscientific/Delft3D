subroutine periodic_xGyG_U(xGU,yGU,nlb,nub,mlb,mub,kmax, gdp)
!----- GPL ---------------------------------------------------------------------
!                                                                               
...
! global variables
!
    real(fp), dimension(nlb:nub,mlb:mub,kmax)     , intent(inout)   :: xGU  
    real(fp), dimension(nlb:nub,mlb:mub,kmax)     , intent(inout)   :: yGU  
...
!     
    ...
    do k = 0, nrPER+1
       if (twoCELLSperiod) THEN
          xGU(nPH_extext(k)+shiftHN,mPH_extext(k)+shiftHM,1:kmax) =  xGU(nPQ_intint(k)+shiftHN,mPQ_intint(k)+shiftHM,1:kmax) + LchanPERprojX 
          xGU(nPQ_extext(k)+shiftQN,mPQ_extext(k)+shiftQM,1:kmax) =  xGU(nPH_intint(k)+shiftQN,mPH_intint(k)+shiftQM,1:kmax) - LchanPERprojX
          yGU(nPH_extext(k)+shiftHN,mPH_extext(k)+shiftHM,1:kmax) =  yGU(nPQ_intint(k)+shiftHN,mPQ_intint(k)+shiftHM,1:kmax) + LchanPERprojY 
          yGU(nPQ_extext(k)+shiftQN,mPQ_extext(k)+shiftQM,1:kmax) =  yGU(nPH_intint(k)+shiftQN,mPH_intint(k)+shiftQM,1:kmax) - LchanPERprojY
       endif
    enddo 
    !
end subroutine periodic_xGyG_U
