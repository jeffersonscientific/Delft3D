      ...
      if (cutcell.gt.0.and.GhostMethod.le.1) then       
         ...
         ! Therefore sud uses the updated velocity u in cucnp.f90, but note that it uses the old discharge qxk  to compute d0k
          ...
          if (free_S1_sud.EQ.0) then
             CALL interpG_ATs1LOCATION(s1,lunscr,Irov,mmax,nmax,nmaxus,kmax,nst,nlb,nub,mlb,mub,nmlb,nmub, gdp) !da eliminare e sostituire con s1=s0 (avevo già interpolato s0 quando uguale a s1)
          endif
