subroutine extrapPERIODICedgeBANK1(gdp)
!----- GPL ---------------------------------------------------------------------
!                                                                               
...
!
         EDGEtypeBANK(:,nPQ_ext(k),mPQ_ext(k)) = EDGEtypeBANK(:,nPH_int(k),mPH_int(k)) 
         EDGEtypeBANK(:,nPH_ext(k),mPH_ext(k)) = EDGEtypeBANK(:,nPQ_int(k),mPQ_int(k))
!
         EDGEtypeBANKerod(:,nPQ_ext(k),mPQ_ext(k)) = EDGEtypeBANKerod(:,nPH_int(k),mPH_int(k)) !can be removed I think since its never used as input before extrapPERIODICedgeBANK2
         EDGEtypeBANKerod(:,nPH_ext(k),mPH_ext(k)) = EDGEtypeBANKerod(:,nPQ_int(k),mPQ_int(k)) !can be removed I think since its never used as input before extrapPERIODICedgeBANK2
!
         EDGElenBANK(:,nPQ_ext(k),mPQ_ext(k)) = EDGElenBANK(:,nPH_int(k),mPH_int(k)) 
         EDGElenBANK(:,nPH_ext(k),mPH_ext(k)) = EDGElenBANK(:,nPQ_int(k),mPQ_int(k))
!
         EDGElenWET(nPQ_ext(k),mPQ_ext(k),:) = EDGElenWET(nPH_int(k),mPH_int(k),:) 
         EDGElenWET(nPH_ext(k),mPH_ext(k),:) = EDGElenWET(nPQ_int(k),mPQ_int(k),:)
!
         ...
         EDGExyBANK(nPQ_ext(k),mPQ_ext(k),:,:,:) = EDGExyBANK(nPH_int(k),mPH_int(k),:,:,:) - SHIFT
         EDGExyBANK(nPH_ext(k),mPH_ext(k),:,:,:) = EDGExyBANK(nPQ_int(k),mPQ_int(k),:,:,:) + SHIFT
!                  
!         EDGExyBANKerod(nPQ_ext(k),mPQ_ext(k),:,:,:) = EDGExyBANKerod(nPH_int(k),mPH_int(k),:,:,:) !can be removed I think since its never used as input before extrapPERIODICedgeBANK2
!         EDGExyBANKerod(nPH_ext(k),mPH_ext(k),:,:,:) = EDGExyBANKerod(nPQ_int(k),mPQ_int(k),:,:,:) !can be removed I think since its never used as input before extrapPERIODICedgeBANK2
!
      ...
      !   gvv_cc(nPH_ext(k),mPH_ext(k)) = gvv_cc(nPQ_int(k),mPQ_int(k))   
         if (twoCELLSperiod) then !ONLY for circular, so no shift by LchanPERprojX  needed
            EDGEtypeBANK(:,nPQ_extext(k),mPQ_extext(k)) = EDGEtypeBANK(:,nPH_intint(k),mPH_intint(k)) 
            EDGEtypeBANK(:,nPH_extext(k),mPH_extext(k)) = EDGEtypeBANK(:,nPQ_intint(k),mPQ_intint(k))
   !
            EDGElenBANK(:,nPQ_extext(k),mPQ_extext(k)) = EDGElenBANK(:,nPH_intint(k),mPH_intint(k)) 
            EDGElenBANK(:,nPH_extext(k),mPH_extext(k)) = EDGElenBANK(:,nPQ_intint(k),mPQ_intint(k))
   !
            EDGElenWET(nPQ_extext(k),mPQ_extext(k),:) = EDGElenWET(nPH_intint(k),mPH_intint(k),:) 
            EDGElenWET(nPH_extext(k),mPH_extext(k),:) = EDGElenWET(nPQ_intint(k),mPQ_intint(k),:)
!
            EDGExyBANK(nPQ_extext(k),mPQ_extext(k),:,:,:) = EDGExyBANK(nPH_intint(k),mPH_intint(k),:,:,:) 
            EDGExyBANK(nPH_extext(k),mPH_extext(k),:,:,:) = EDGExyBANK(nPQ_intint(k),mPQ_intint(k),:,:,:)
         endif
      enddo
!
RETURN
end subroutine extrapPERIODICedgeBANK1
