subroutine comvol_onlyVOL(nmmax     ,kmax      ,zmodel    ,kcs                  , &
                        & thick     ,gsqs      ,dps       ,s1        ,volum1    , &
                        & dzs1      ,agsqs_inpt,kfs       ,gdp       )
!----- GPL ---------------------------------------------------------------------
!                                                                               
...
!!--description-----------------------------------------------------------------
!
!    Function: Computes non'linked volumes from depth values at water level
...
!    real(fp), dimension(gdp%d%nmlb:gdp%d%nmub)      , intent(in)  :: aguu      !  Description and declaration in esm_alloc_real.f90
    real(fp), dimension(gdp%d%nmlb:gdp%d%nmub)      , intent(in)  :: agsqs_inpt
...
if (.not.zmodel) then
   do k = 1, kmax
      do nm = 1, nmmax
         volum1(nm, k) = thick(k)*(s1(nm) + real(dps(nm),fp))*gsqs(nm) * agsqs_inpt(nm) * masks(nm)
      enddo
   enddo
else
   !
   ...
   do k = 1, kmax
      do nm = 1, nmmax
         volum1(nm, k) = dzs1(nm, k)*gsqs(nm)*agsqs_inpt(nm)*masks(nm)
      enddo
   enddo
endif
deallocate(masks)
!deallocate(masks,masku)
end subroutine comvol_onlyVOL
