subroutine BOUvelFROMdisch(kfumn0,kfumx0,dzu0,dzmin,thick,u1,qxk,momsol,kfu,circ3d,hu,aguu,guu,irocol,norow,icx,icy,nmmax,nmlb,nmub,kmax,nst,ddb,wavcmp,Zmodel,gdp) 
...
!
             if (MERGEDwith_d(nmfu)>0.or.FORCEdisch) then !  donor small cell if  (MERGEDwith_d(nmfu)>0)
                ...
                do k = k0f, k1f
                   ...
                   u1(nmf, k)  = circ3d(k, 1, ic)/(guu(nmf)*aguu(nmf)*relthk)
                enddo
             endif
!
          elseif (ibf==8) then
...
!
             if (MERGEDwith_d(nml)>0.or. FORCEdisch) then !  donor small cell if  (MERGEDwith_d(nml)>0)
                ...
                do k = k0l, k1l
                   ...
                   u1(nml, k)  = circ3d(k, 2, ic)/(guu(nml)*aguu(nml)*relthk)
                enddo
             endif
!
...
!
RETURN
end subroutine BOUvelFROMdisch
