      subroutine s0iss1_ghost(s0,s1,nlb,nub,mlb,mub,kmax,IGNOREactive, gdp) 
!----- GPL ---------------------------------------------------------------------
!                                                                               
...
!
    integer           :: i,mGP,nGP,IGNOREactive
!
   ...
   !
    do i = 1,totGHOSTs1
      mGP = mGPs1(i)
      nGP = nGPs1(i) 
      if ((comparereal(poros(nGP,mGP),0._fp).gt.0).and.IGNOREactive==1) cycle   
      s0(nGP,mGP) = s1(nGP,mGP)
    enddo

  return 
end subroutine s0iss1_ghost
