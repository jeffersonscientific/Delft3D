...
!!--description-----------------------------------------------------------------
!
!   Function:   periodic porosity at periodic boundaries 
!               
...
!
    ...
    mPH_ext => gdp%gdimbound%mPH_ext
      ...
      do k=1,nrPER 
!        
         poros(nPQ_ext(k),mPQ_ext(k)) = poros(nPH_int(k),mPH_int(k))
         poros(nPH_ext(k),mPH_ext(k)) = poros(nPQ_int(k),mPQ_int(k))
!
      enddo
!
RETURN
end subroutine PER_poros
