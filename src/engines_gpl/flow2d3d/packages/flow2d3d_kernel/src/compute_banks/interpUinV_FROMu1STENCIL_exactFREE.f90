subroutine interpUinV_FROMu1STENCIL_exactFREE(u,v,uintv,kcs,lunscr,mmax,nmax,nmaxus,kmax,nst,nlb,nub,mlb,mub,nmlb,nmub,exitloop,iterFR, gdp)
!----- GPL ---------------------------------------------------------------------
!                                                                               
...
!
    ...
    real(fp), dimension(nlb:nub,mlb:mub,kmax)                           , intent(inout) :: uintv
...
!
   ...
   !         extrapolate vertically 
    ...
    if (periodSURFACE.and.PERIODalongM==0)  CALL periodic_u0INTv(v,nlb,nub,mlb,mub,kmax, gdp)  !interpolate v0INTu,since the _extext tangential point cannot find a u value by interpolation, therefore it has to be copied
!
    do n=1,nmax
      do m=1,mmax
      if (inSTENCILv(n,m) == 1) then

       iniVALUE(1:kmax) = uintv(n,m,1:kmax)       
!        
      ...
      ! lower left  
       ...
       Do K=1,4
         ...
         ELSEIF (GHOSTu1(nL(K)+1,mL(K)) == 1) then ! dry ghost cell ! note here n is increased by one because the location of xcorV1(n,m) is the location of xG_u1(n+1,m)
            ...
            kGHOS(contGHOST) = K
            if (contGHOST==1) then ! I need a normal for computing dutdn_U, I use the first I get (the average woudl have been fine also, for linear boundary its the same)
               i  = FROMmnTOghostU1(nL(K)+1,mL(K))  
            endif  
...
!
      ...
      elseif (freeNONhomo==1) then 
         ...
         elseif (contFLUID+contGHOST==4) then
           ...
           ! write(*,*) 'check if normal goes toward dry otherwise change sign'
            write(1000003,'(4i8,f25.15)') iterFR,i,m,n,dutdn_U(n,m,1)
      ...
      elseif (freeNONhomo==5) then !exact solution circular channel !2D
         ...
         if (subtypeTESTghost==3) then ! anular channel 
            if (xG_V1(n,m)**2+yG_V1(n,m)**2>3600._fp ) then ! note I use V1 instead of U1, because there could be no ghost in the stencil. 
         ...
         elseif (subtypeTESTghost==2) then ! straight channel 43/80, slope 0.001 orthogonal
            if ( yG_V1(n,m)>0.525_fp*xG_V1(n,m)+250._fp ) then  !note I use V1 instead of U1, because there could be no ghost in the stencil. 
...
!
          ...
          DO K=1,kmax
             ...
             if (.NOT.solvedU_ATu) THEN
                ...
                uintv(n,m,k) = B(1)*xP*yP+B(2)*xP+B(3)*yP+B(4)              
             endif
           
             if ( uintv(n,m,k).lt.-1000.or.uintv(n,m,k).gt.1000 ) then
...
!            INSTEAD OF DOING A CYCLE CALL ONCE DGETRS CREATING B8(1:8,1:KMAX) AND calling CALL CALL DGETRS('N', 8, 8 ,.... instead of DGETRS('N', 8, 1
   ...
   !  
                ...
                CALL ERROR_DGETRS(INFO,contFLUID,mBI,nBI)     
                uintv(n,m,k) = B8(1)*xP*yP+B8(3)*xP+B8(5)*yP+B8(7)
   !
                if (uintv(n,m,k) .lt.-1000.or.uintv(n,m,k) .gt.1000) then
                  write(*,*) ' wrong value of u in n,m,nst', n,m,nst
                  call d3stop(1, gdp)
...
!
            do k=1,kmax
               ...
               if (.NOT.solvedU_ATu) THEN      
                  ...
                  CALL ERROR_DGETRS(INFO,contFLUID,mBI,nBI)  
                  uintv(n,m,k) = B3(1)*xP+B3(2)*yP+B3(3) 
               endif
...
!
               ...
               uintv(n,m,1:kmax) = (u(nL(k1)+1,mL(k1),1:kmax) + u(nL(k4)+1,mL(k4),1:kmax))*0.5_fp  !(B8(1)+B8(7))*0.5_fp               
            endif
 
         ELSE ! IF(setTOzero) 
           !average of fluid points
            uintv(n,m,1:kmax) = (u(nL(k1)+1,mL(k1),1:kmax) + u(nL(k4)+1,mL(k4),1:kmax) ) *0.5_fp
         ENDIF
!
! 
...
!
   ...
   ! 
                   ...
                   CALL ERROR_DGETRS(INFO,contFLUID,mBI,nBI)   
                   uintv(n,m,k) = B6(1)*xP+B6(3)*yP+B6(5) 
                   if (uintv(n,m,k).lt.-1000.or.uintv(n,m,k).gt.1000) then
                     write(*,*) ' wrong value of u in n,m,nst', n,m,nst
                     call d3stop(1, gdp)
                   endif  
   ...
   !
                ...
                write(*,*) 'The two normals for u1 are coincident for  point (m,n)=,',m,n,'at time step ',nst
                !call d3stop(1,gdp)  !uncomment this to set the value constant in the cell
                uintv(n,m,1:kmax) = u(nL(k3)+1,mL(k3),1:kmax) 
             endif
...
!          write(1232111,'(4i8,10f25.15)') iterFR,n,m,K,iniVALUE(k),uintv(n,m,k)
!          if (  abs(iniVALUE(k) - uintv(n,m,k)) > 0.000001_fp) then
!             write(2232111,'(4i8,10f25.15)') iterFR,n,m,K,iniVALUE(k),uintv(n,m,k)
!             exitloop = 0
!             exit loop_k_1
!          endif
!       enddo loop_k_1      
