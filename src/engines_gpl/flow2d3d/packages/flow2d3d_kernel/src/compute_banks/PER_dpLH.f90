subroutine PER_dpLH(depth, dzVECT, nlb, mlb, nub, mub, nrPER, gdp) 
!----- GPL ---------------------------------------------------------------------
!                                                                               
...
! 
    ...
    real(fp), dimension(nlb:nub,mlb:mub)  , intent(out) :: depth      
    real(fp)                              , intent(in)  :: dzVECT(1:nrPER)
!
! local variables
!
    integer                    :: k 
...
!
    ...
    !
    do k=1,nrPER  
       depth(nPQ_ext(k),mPQ_ext(k)) = depth(nPH_int(k),mPH_int(k)) - dzVECT(k)  
       depth(nPH_ext(k),mPH_ext(k)) = depth(nPQ_int(k),mPQ_int(k)) + dzVECT(k)     
    enddo
    !
end subroutine PER_dpLH
