...
!    Reference:
!    Review:
      SUBROUTINE GREGORSbk ( JULIAN, IYEAR , IMONTH, IDAY  , IHOUR , &
                             IMIN  , ISEC  )
!
!     +----------------------------------------------------------------+
...
!
      ...
      ELSE
         ...
         DO WHILE ( nTry <= 2 )
             ...
             TEMP1  = TEMP1 - DINT ( ( 146097.0 * TEMP2 + 3.0D0 ) / 4.0 )
             IYEAR  = INT   ( 4000.0 * ( TEMP1 + 1.0 ) / 1461001.0 )
             TEMP1  = TEMP1 - DINT ( (1461.0D0 * IYEAR) / 4.0D0 ) + 31.0
