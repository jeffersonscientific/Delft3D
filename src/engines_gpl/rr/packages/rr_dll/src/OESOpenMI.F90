...
function DefineExchItemIdsElmset_OES(componentID , schemID         , &
    ...
    character(Len=oes_id_len), dimension(elementCount), intent(in) :: ElementIDs
...
function DefineWrapperElmset_OES(componentID , schemID         , &
    ...
    ! locals
    character(Len=oes_id_len), pointer, dimension(:) :: ids          ! element ids
    ...
    if ( modelHandle > 0 ) then
        ...
        numElms = size(ids)
        retVal = DefineExchItemIdsElmset_OES(componentID, schemID, quantityID, role, elementsetID, ids, numElms, oes_id_len)
        if (retVal == 0) then
            retVal = numElms
        endif
    endif
