...
from ._dsle_cffi import ffi, lib
...
def _dsle_error_message(code):
    return ffi.string(lib.dsle_error_msg(code)).decode("utf-8")


def _dsle_version():
    return ffi.string(lib.dsle_version()).decode("utf-8")


def dsle_calc_steady(auxiliary_results: bool = False, **parameters: float) -> Dict[str, float]:
    """
    ...
    :param auxiliary_results: Whether or not to calculate and output auxiliary
        results. See :c:struct:`dsle_aux_results_t`.
    :param kwargs: Any parameters that should be changed versus the default.
        See also :c:struct:`dsle_param_t` for an overview of the parameters.

    :returns: A dictionary containing the cycle averaged salt fluxes and
        discharges (see :c:struct:`dsle_results_t`). Also outputs values in
        :c:struct:`dsle_aux_results_t` if ``auxiliary_results`` is `True`.
    """
    param_t = ffi.new("dsle_param_t *")
    ...
    # Set default values
    lib.dsle_param_default(param_t)
    ...
    # Get results
    results_t = ffi.new("dsle_results_t *")
    if auxiliary_results:
        aux_results_t = ffi.new("dsle_aux_results_t *")
    else:
        aux_results_t = ffi.NULL
        assert len(dir(aux_results_t)) == 0

    err = lib.dsle_calc_steady(param_t, results_t, aux_results_t)
...
class DSLEUnsteady:
    ...
    def __init__(self, sal_lock, head_lock, **parameters: float):
        ...
        self._param_t = ffi.new("dsle_param_t *")
        self._state_t = ffi.new("dsle_phase_state_t *")
        # We can reuse the same object for results,
        # as we convert it to a dictionary before returning
        self._results_t = ffi.new("dsle_phase_transports_t *")

        self._param_t_names = set(dir(self._param_t))

        # Set default values
        lib.dsle_param_default(self._param_t)

        # Set user parameters
        self._set_parameters(**parameters)

        # Initialize the state
        lib.dsle_initialize_state(self._param_t, self._state_t, sal_lock, head_lock)
    ...
    def step_phase_1(self, t_level, **parameters: float) -> Dict[str, float]:
        """
        Level the lock to lake side. See also :c:func:`dsle_step_phase_1` .
        ...
        :returns: The salt and water transports in this phase.
                  See also :c:struct:`dsle_phase_transports_t`.
        """

        self._set_parameters(**parameters)

        err = lib.dsle_step_phase_1(self._param_t, t_level, self._state_t, self._results_t)
        if err:
            raise RuntimeError(_dsle_error_message(err))
    ...
    def step_phase_2(self, t_open_lake: float, **parameters: float) -> Dict[str, float]:
        """
        Open the door on lake side. See also :c:func:`dsle_step_phase_2` .
        ...
        err = lib.dsle_step_phase_2(self._param_t, t_open_lake, self._state_t, self._results_t)
        if err:
            raise RuntimeError(_dsle_error_message(err))
    ...
    def step_phase_3(self, t_level, **parameters: float) -> Dict[str, float]:
        """
        Level the lock to sea side. See also :c:func:`dsle_step_phase_3` .
        ...
        err = lib.dsle_step_phase_3(self._param_t, t_level, self._state_t, self._results_t)
        if err:
            raise RuntimeError(_dsle_error_message(err))
    ...
    def step_phase_4(self, t_open_sea: float, **parameters: float) -> Dict[str, float]:
        """
        Open the door on sea side. See also :c:func:`dsle_step_phase_4` .
        ...
        err = lib.dsle_step_phase_4(self._param_t, t_open_sea, self._state_t, self._results_t)
        if err:
            raise RuntimeError(_dsle_error_message(err))
    ...
    def step_flush_doors_closed(self, t_flushing: float, **parameters: float) -> Dict[str, float]:
        """
        Open the door on sea side. See also :c:func:`dsle_step_flush_doors_closed` .
    ...
    def state(self) -> Dict[str, float]:
        """
        Get the state of the lock, see also :c:struct:`dsle_phase_state_t`.
