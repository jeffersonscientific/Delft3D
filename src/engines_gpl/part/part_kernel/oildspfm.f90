...
contains
    subroutine oildspfm (itime)
        ...
        integer  (int_wp) :: id, ifrac, isuboil, jsub   ! help and loop variables for dyes, fractions and substances
        ...
        if (first) then
            ...
            write(lun(2), '(//)')
            do ifrac = 1, nfract
                if (ioptev(ifrac)>=0)write(lun(2), *) ' Fraction ', ifrac, ' evaporized: ', ioptev(ifrac), ' per day ' !only when it is a evaporation constant
        ...
        do i = 1, Nopart  ! here we loop the particles TODO: check the start end end, because in fm npdndw may not exist.
            ...
            if (nstick > 0) then
                wsum = 0.0
                do isuboil = 1, nosubs
                    if (mstick(isuboil) < 0) then
                        wsum = wsum + wpart(isuboil, i)
                        ifrac = isuboil / 3
                        if (3 * ifrac /= isuboil) then
            ...
            if (ic > 0) then !che
                ...
                if (mpart(i) <= 0 .or. mpart(i) > numcells) then
                ...
                do ifrac = 1, nfract
                    if  (wpart(ioilt(ifrac), i) > 0) then ! this should be referring to the surface layer (ie when that part has a weight > then there is surface floating oil)
