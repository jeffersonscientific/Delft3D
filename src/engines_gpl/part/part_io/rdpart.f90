    ...
    subroutine rdpart (lun1, lun2, lnam1)
        ...
        if (alone) then                 ! clean initialization
            ...
            do i = 1, lstack              ! at the first free entry
                if (ilun(i) /= 0 .and. lch (i) == lnam1) exit
                if (ilun(i) /= 0) cycle
                ilun(i) = 900 + i
                lch (i) = lnam1
                exit
        ...
        if (gettoken(filvers, ierr2) /= 0) goto 4001
        if (filvers(1:19) == 'delpar_version_3.60') filvers(1:19) = 'v3.60.00'
        ...
        if (gettoken(title(1), ierr2) /= 0) goto 4002
        if (gettoken(title(2), ierr2) /= 0) goto 4002
        if (gettoken(title(3), ierr2) /= 0) goto 4002
        if (gettoken(title(4), ierr2) /= 0) goto 4002
        write (lun2, 2000) title(1)
        write (*, 2000) title(1)

        !     read the name of the hyd file for ui delft3d (ignored here for now)

        if (gettoken(cbuffer, ierr2) /= 0) goto 4003
        ...
        if (gettoken(ipc, ierr2) /= 0) goto 4007
        if (gettoken(idelt, ierr2) /= 0) goto 4007
        ...
        if (gettoken(ioptdv, ierr2) /= 0) goto 4008
        if (gettoken(alpha, ierr2) /= 0) goto 4008
        if (gettoken(cdisp, ierr2) /= 0) goto 4008
        select case (ioptdv)
        case (0)
            if (alpha > 0.0 .and. cdisp > 0.0) then
                write (lun2, *) '   '
        ...
        case (2)
            if (gettoken(dminim, ierr2) /= 0) goto 4008
            if (alpha > 0.0 .and. dminim >= 0.0) then
                write (lun2, *) '   '
        ...
        if (nolayp > 1 .and. modtyp /= model_two_layer_temp) then
            write (lun2, 3115)
        ...
        if (gettoken(nosubs, ierr2) /= 0) goto 4009
        nfract = 0
        if (oil) then
            if (gettoken(nfract, ierr2) /= 0) goto 4009
        endif
        ...
        do i = 1, nosubs
            if (gettoken(substi(i), ierr2) /= 0) goto 4009
        ...
        do isb = 1, nosubs                 !.. substances that are sticking
            if (isb > (3 * nfract) .and. substi(isb)(1:6) == "stick_") then
                do jsub = 1, nosubs           !.. substances that are sticking and that are not oil
                    if (substi(jsub)(1:14) == substi(isb)(7:20)) then
                        mstick(jsub) = isb
                        nstick = nstick + 1
        ...
        if (gettoken(nopart, ierr2) /= 0) goto 4013         !     number of particles
        write (lun2, 2020) nopart
        write (*, 2020) nopart

        if (gettoken(rough, ierr2) /= 0) goto 4014          !     roughness
        write (lun2, 2040) rough

        if (gettoken(drand(1), ierr2) /= 0) goto 4015       !     random walk
        if (gettoken(drand(2), ierr2) /= 0) goto 4015
        ...
        if (gettoken(drand(3), ierr2) /= 0) goto 4016       !     wind fetch [%]
        write (lun2, 2060) drand(3)

        if (gettoken(rhow, ierr2) /= 0) goto 40161          !     water density
        write (lun2, 2065) rhow

        !     constant wind velocity and direction
        if (gettoken(cbuffer, nowind, itype, ierr2) /= 0) goto 4017    !     number of wind breakpoints
        if (itype == 1) then                                  ! a character giving a meteo steering file
            ...
            do i = 1, nowind                                          !     wind varies with the time
                if (gettoken(id, ierr2) /= 0) goto 4018
                if (gettoken(ih, ierr2) /= 0) goto 4018
                if (gettoken(im, ierr2) /= 0) goto 4018
                if (gettoken(is, ierr2) /= 0) goto 4018
                if (gettoken(wveloa(i), ierr2) /= 0) goto 4018
                if (gettoken(wdira (i), ierr2) /= 0) goto 4018
        ...
        if (gettoken(noconsp, ierr2) /= 0) goto 4019

        if (.not. oil) then
            if (noconsp > 0) call alloc ("const", const, noconsp)
            do i = 1, noconsp
                if (gettoken(const(i), ierr2) /= 0) goto 4020
            enddo
        else
            ...
            do ifract = 1, nfract
                ...
                if (gettoken(const(i), ierr2) /= 0) goto 4020      ! evaporation
                if (const(i)<-0.5) then
                    if (const(i)<-1.5) then
                        write(lun2, '(8x,a)')'Effect of water content on evaporation invoked'
                    endif
                    i = i + 1
                    if (gettoken(const(i), ierr2) /= 0) goto 4020      ! evaporation d180 parameter
                    if (const(i)>0) then
                    ...
                    i = i + 1
                    if (gettoken(const(i), ierr2) /= 0) goto 4020      ! temperature of the evaporation Fingas process
                ...
                i = i + 1
                if (gettoken(const(i), ierr2) /= 0) goto 4020      ! dispersion option
                if (const(i) == 0) then
                    i = i + 1
                    if (gettoken(const(i), ierr2) /= 0) goto 4020   ! dispersion rate/day
                    write (lun2, '(8x,a,es15.7)') 'Dispersion rate (per day)                        : ', const(i)    !  3
                elseif (const(i)==1) then
                    i = i + 1
                    const(i) = 0.0                                        ! not used
                    write (lun2, '(8x,a,es15.7)') 'Dispersion C0 from Delvigne/Sweeney formula'
                elseif (const(i)==2) then
                    i = i + 1
                    if (gettoken(const(i), ierr2) /= 0) goto 4020   ! dispersion rate/day
            ...
            i = i + 1
            if (gettoken(const(i), ierr2) /= 0) goto 4020    ! minimum thickness oil layer
            write (lun2, '(8x,a,e10.3)') 'Minimum thickness oil layer                      : ', const(i)       ! 11
            if (nolayp > 1) then
                i = i + 1
                if (gettoken(const(i), ierr2) /= 0) goto 4020 ! deflection angle
                write (lun2, '(8x,a,f10.3)') 'Deflection angle (Coriolis - 3D only)            : ', const(i)       ! 12
            endif
            stickdf = 0
            if (filvers(2:19) >= '3.73.00') then
                if (gettoken(stickdf, ierr2) /= 0) goto 4020       ! sticking at drying flats
            endif
            if (stickdf == 0) then
      ...
      leeway_angle = 0.0
        ...
        if (gettoken(cbuffer, id, itype, ierr2) /= 0) then
            if (ierr2 == 2) then
                ...
                if (gettoken(id, ierr2) /= 0) goto 4021
            else
            end if
        else
            if (itype == 1) then
                do while (itype == 1)
                    ...
                    case ('max_restart_age')
                        ...
                        if (gettoken (max_restart_age, ierr2) /= 0) goto 9010
                        if (max_restart_age==0) goto 9011
                    ...
                    case ('plastics_parameters')
                        ...
                        if (gettoken(cplastic, ierr2) /= 0) goto 9103
                        do while (cplastic /= 'end')
                            write (lun2, 3501) trim(cplastic)
                            if (gettoken(rdpldensity, ierr2) /= 0) goto 9104
                            write (lun2, 3502) rdpldensity
                            if (gettoken(rdplshapefactor, ierr2) /= 0) goto 9104
                            write (lun2, 3503) rdplshapefactor
                            if (gettoken(rdplmeansize, ierr2) /= 0) goto 9104
                            write (lun2, 3504) rdplmeansize
                            if (gettoken(rdplvarsize, ierr2) /= 0) goto 9104
                            write (lun2, 3505) rdplvarsize
                            if (gettoken(rdplfragrate, ierr2) /= 0) goto 9104
                            write (lun2, 3506) rdplfragrate
                            if (rdplmeansize <= 0.0) goto 9105
                            rdplmusize = log((rdplmeansize**2) / sqrt(rdplvarsize + rdplmeansize**2))
                            rdplsigmasize = sqrt(log(rdplvarsize / (rdplmeansize**2) + 1))
                            isb = -1
                            do i = 1, nosubs
                                if(cplastic == substi(i)) then
                                    isb = i
                                    exit
                                endif
                            enddo
                            if(isb > 0) then
                                if (plparset(isb) == 1) goto 9106
                    ...
                    case ('screens')
                        ...
                        screens = .true.
                        if (gettoken(permealeft, ierr2) /= 0) goto 9201   ! leftside permeability of screeens
                        if (gettoken(permearight, ierr2) /= 0) goto 9202   ! rightside permeability of screeens
                        if (gettoken(fiscreens, ierr2) /= 0) goto 9203   ! screens polygon

                        open (newunit = lunfil, file = fiscreens, status = 'old', iostat = ierr2)
                        if (ierr2 /= 0) go to 9204
                        call getdim_dis (lunfil, fiscreens, nrowsscreens, lun2)
                        close (lunfil)
                        if (nrowsscreens > 0) then
                    ...
                    case ('partinifile')
                        ...
                        if (gettoken(partinifile, ierr2) /= 0) goto 9301   ! part FM ini file
                    case ('partrelfile')
                        write (lun2, '(/a)') '  Found keyword "partrelfile".'
                        if (gettoken(partrelfile, ierr2) /= 0) goto 9302   ! part FM ini file
                    case ('abmmodel')
                        ...
                        if (gettoken(abmstagedev, ierr2) /= 0) goto 9404   ! ABM stage development
                    ...
                    if (gettoken(cbuffer, id, itype, ierr2) /= 0) goto 4021
                end do
            end if
        end if
        ...
        if (gettoken(ih, ierr2) /= 0) goto 4021
        if (gettoken(im, ierr2) /= 0) goto 4021
        if (gettoken(is, ierr2) /= 0) goto 4021
        write (lun2, 2090) id, ih, im, is
        itstrtp = id * 86400 + ih * 3600 + im * 60 + is

        !     simulation stop  time

        if (gettoken(id, ierr2) /= 0) goto 4022
        if (gettoken(ih, ierr2) /= 0) goto 4022
        if (gettoken(im, ierr2) /= 0) goto 4022
        if (gettoken(is, ierr2) /= 0) goto 4022
        write (lun2, 2110) id, ih, im, is
        itstopp = id * 86400 + ih * 3600 + im * 60 + is
        if (mod(itstopp - itstrtp, idelt) /=  0) then
        ...
        if (gettoken(id, ierr2) /= 0) goto 4023
        if (gettoken(ih, ierr2) /= 0) goto 4023
        if (gettoken(im, ierr2) /= 0) goto 4023
        if (gettoken(is, ierr2) /= 0) goto 4023
        ...
        if (gettoken(id, ierr2) /= 0) goto 4024
        if (gettoken(ih, ierr2) /= 0) goto 4024
        if (gettoken(im, ierr2) /= 0) goto 4024
        if (gettoken(is, ierr2) /= 0) goto 4024
        write (lun2, 2130) id, ih, im, is
        icwsta = id * 86400 + ih * 3600 + im * 60 + is
        if (icwsta < itstrtp) then
            write(lun2, 2140)
            ierr = ierr + 1
        endif
        ...
        if (gettoken(id, ierr2) /= 0) goto 4025
        if (gettoken(ih, ierr2) /= 0) goto 4025
        if (gettoken(im, ierr2) /= 0) goto 4025
        if (gettoken(is, ierr2) /= 0) goto 4025
        ...
        if (gettoken(id, ierr2) /= 0) goto 4026
        if (gettoken(ih, ierr2) /= 0) goto 4026
        if (gettoken(im, ierr2) /= 0) goto 4026
        if (gettoken(is, ierr2) /= 0) goto 4026
        ...
        if (gettoken(id, ierr2) /= 0) goto 4027
        if (gettoken(ih, ierr2) /= 0) goto 4027
        if (gettoken(im, ierr2) /= 0) goto 4027
        if (gettoken(is, ierr2) /= 0) goto 4027
        write (lun2, 2131) id, ih, im, is
        ihstrtp = id * 86400 + ih * 3600 + im * 60 + is
        if (ihstrtp < itstrtp) then
            write(lun2, 2166)
            ierr = ierr + 1
        endif
        ...
        if (gettoken(id, ierr2) /= 0) goto 4028
        if (gettoken(ih, ierr2) /= 0) goto 4028
        if (gettoken(im, ierr2) /= 0) goto 4028
        if (gettoken(is, ierr2) /= 0) goto 4028
        write (lun2, 2151) id, ih, im, is
        ihstopp = id * 86400 + ih * 3600 + im * 60 + is
        if (ihstopp < ihstrtp) then
            write(lun2, 2167)
            ierr = ierr + 1
        endif
        ...
        if (gettoken(id, ierr2) /= 0) goto 4029
        if (gettoken(ih, ierr2) /= 0) goto 4029
        if (gettoken(im, ierr2) /= 0) goto 4029
        if (gettoken(is, ierr2) /= 0) goto 4029
        ...
        if (gettoken(iyear, ierr2) /= 0) goto 4030
        if (gettoken(imonth, ierr2) /= 0) goto 4030
        if (gettoken(id, ierr2) /= 0) goto 4030
        if (gettoken(ih, ierr2) /= 0) goto 4030
        if (gettoken(im, ierr2) /= 0) goto 4030
        if (gettoken(is, ierr2) /= 0) goto 4030
        ...
        if (oil) then
            ...
            if (tydisp > 0) then
                if (tydisp /= 1) goto 6005
                !     read number of dispersant events
                if (gettoken(ndisapp, ierr2) /= 0) goto 6002
                write(*, 3300) ndisapp
                ...
                if (ndisapp > 0) then
                    ...
                    do i = 1, ndisapp
                        ...
                        if (gettoken(id, ierr2) /= 0) goto 6003
                        if (gettoken(ih, ierr2) /= 0) goto 6003
                        if (gettoken(im, ierr2) /= 0) goto 6003
                        if (gettoken(is, ierr2) /= 0) goto 6003
                        !     read dispersant parameterisation
                        do ifract = 1, nfract
                            if (gettoken(efdisp(i, ifract), ierr2) /= 0) goto 6006
                            if (tydisp == 1) then
                                if (efdisp(i, ifract) < 0 .or. efdisp(i, ifract) > 1.0) goto 6007
                            end if
                        end do
                        !     read dispersant polygon file
                        if (gettoken(fidisp(i), ierr2) /= 0) goto 6010
                        !     determine maximum no. of rows for polygons (nrowsmax)
                        open (newunit = lunfil, file = fidisp(i), status = 'old', iostat = ierr2)
                        if (ierr2 /= 0) go to 1700
                        ...
                        if (idisset(i) < itstrtp .or. idisset(i) > itstopp) then
                            write (*, 3320)
                            write (lun2, 3320)
                            ierr = ierr + 1
                        endif
                        if (mod(idisset(i), idelt) /= 0) then
                            write (*, 3321)
                            write (lun2, 3321)
                            ierr = ierr + 1
                        endif
                        if (i > 1) then
                            if (idisset(i) <= idisset(i - 1)) then
                                write (*, 3322)
                                write (lun2, 3322)
                                ierr = ierr + 1
                            endif
            ...
            if (tyboom > 0) then
                if (tyboom /= 1) goto 6025
                !     read number of boom introductions
                if (gettoken(nboomint, ierr2) /= 0) goto 6022
                write(*, 3301) nboomint
                ...
                if (nboomint > 0) then
                    ...
                    do i = 1, nboomint
                        ...
                        if (gettoken(id, ierr2) /= 0) goto 6023
                        if (gettoken(ih, ierr2) /= 0) goto 6023
                        if (gettoken(im, ierr2) /= 0) goto 6023
                        if (gettoken(is, ierr2) /= 0) goto 6023
                        do ifract = 1, nfract
                            if (gettoken(efboom(i, ifract), ierr2) /= 0) goto 6026
                            if (tyboom == 1) then
                                if (efboom(i, ifract) < 0 .or. efboom(i, ifract) > 1.0) goto 6027
                            end if
                        end do
                        if (gettoken(fiboom(i), ierr2) /= 0) goto 6010
                        !     determine maximum no. of rows for polygones (nrowsmax)
                        open (newunit = lunfil, file = fiboom(i), status = 'old', iostat = ierr2)
                        if (ierr2 /= 0) go to 1701
                        call getdim_dis (lunfil, fiboom(i), nrowsmax, lun2)
                        close (lunfil)

                        !     check for ascending order of events, and one per timestep
                        iboomset (i) = id * 86400 + ih * 3600 + im * 60 + is
                        if (iboomset(i) < itstrtp .or. iboomset(i) > itstopp) then
                            write (*, 3320)
                            write (lun2, 3320)
                            ierr = ierr + 1
                        endif
                        if (mod(iboomset(i), idelt) /= 0) then
                            write (*, 3321)
                            write (lun2, 3321)
                            ierr = ierr + 1
                        endif
                        if (i > 1) then
                            if (iboomset(i) <= iboomset(i - 1)) then
                                write (*, 3322)
                                write (lun2, 3322)
                                ierr = ierr + 1
                            endif
            ...
            if (ndisapp > 0 .and. nrowsmax > 0) then
                !     allocate memory for the dispersant polygons, and read them into memory
        ...
        idp_file = " "
        nopart_res = 0
        if (gettoken(cbuffer, nosta, itype, ierr2) /= 0) goto 4031
        if (itype == 1) then
            idp_file = cbuffer
            if (idp_file /= ' ') then
            ...
            if (gettoken(nosta, ierr2) /= 0) goto 4031
        endif
        if (ihstepp <= 0 .and. nosta > 0) then
            write (lun2, 2024)
            ierr = ierr + 1
        endif
        i = max(nosta, 1)
        ...
        do i = 1, nosta
            if (gettoken(nmstat(i), ierr2) /= 0) goto 4032
            if (gettoken(xstat (i), ierr2) /= 0) goto 4032
            if (gettoken(ystat (i), ierr2) /= 0) goto 4032
            write (lun2, 2027) nmstat(i), xstat(i), ystat(i)
        enddo

        !     read the plot characteristics

        if (gettoken(iptset, ierr2) /= 0) goto 4033
        if (iptset > 0) then
        ...
        do i = 1, iptset
            if (gettoken(id, ierr2) /= 0) goto 4034
            if (gettoken(ih, ierr2) /= 0) goto 4034
            if (gettoken(im, ierr2) /= 0) goto 4034
            if (gettoken(is, ierr2) /= 0) goto 4034
            if (gettoken(recovr(i), ierr2) /= 0) goto 4034
            write (lun2, 2210) id, ih, im, is, recovr(i)
            ipset (i) = id * 86400 + ih * 3600 + im * 60 + is
            if (ipset(i) < itstrtp .or. ipset(i) > itstopp) then
                write (lun2, 2220)
                ierr = ierr + 1
            endif
            if (i > 1) then
        ...
        npgrid = 1
        if (gettoken(i, xw1f, itype, ierr2) /= 0) goto 4038
        if (itype == 2) then
            npgrid = i
        ...
        do i = 1, npgrid
            if (npgrid > 1) write (lun2, '(/A,i4)') 'Information for plotgrid: ', i
            if (itype == 2) then
                if (gettoken(xw1f, ierr2) /= 0) goto 4038
            endif
            if (gettoken(xw2f, ierr2) /= 0) goto 4038
            if (xw1f > xw2f) then     !       ascending order; swap
                xw1f = xw1f + xw2f
                xw2f = xw1f - xw2f
                xw1f = xw1f - xw2f
            endif
            ...
            if (gettoken(yw1f, ierr2) /= 0) goto 4038
            if (gettoken(yw2f, ierr2) /= 0) goto 4038
            if (yw1f > yw2f) then     !       ascending order; swap
                yw1f = yw1f + yw2f
                yw2f = yw1f - yw2f
                yw1f = yw1f - yw2f
            endif
            ...
            if (gettoken(cbuffer, pg(i)%mmap, itype, ierr2) /= 0) goto 4038
            pg(i)%ztype = 0
            if (itype == 1) then
                if (gettoken(xw1f, ierr2) /= 0) goto 4038
                if (gettoken(xw2f, ierr2) /= 0) goto 4038
                ...
                if (gettoken(pg(i)%mmap, ierr2) /= 0) goto 4038
            endif

            !     resolution of the plot window

            if (gettoken(pg(i)%nmap, ierr2) /= 0) goto 4038
        ...
        if (gettoken(nodye, ierr2) /= 0) goto 4039
        i = max(nodye, 1)
        ...
        do i = 1, nodye
            ...
            if (gettoken(nmdyer(i), ierr2) /= 0) goto 4040
            write (lun2, 2258) nmdyer(i)

            !       time of the instantaneous release

            if (gettoken(id, ierr2) /= 0) goto 4041
            if (gettoken(ih, ierr2) /= 0) goto 4041
            if (gettoken(im, ierr2) /= 0) goto 4041
            if (gettoken(is, ierr2) /= 0) goto 4041
            write (lun2, 2260) id, ih, im, is
            iwtime(i) = id * 86400 + ih * 3600 + im * 60 + is
            if (iwtime(i) < itstrtp .or. iwtime(i) > itstopp) then
                write (lun2, 2270)
                ierr = ierr + 1
            endif
         ...
         if ( .not. oil .and. .not. leeway) then
            ...
            if (gettoken(ioptrad(i), ierr2) /= 0) goto 4043
            if (ioptrad(i) == 0) then
                ! read all tokens
                if (gettoken(fidye(i), ibuffer, radius(i), itype, ierr2) /= 0) goto 4043
                if (itype==2) then
            ...
            if (gettoken(wparm(i), ierr2) /= 0) goto 4043
            if (wparm(i) >= 0.0) then
            ...
            write (lun2, 2289)
            do k = 1, nosubs
                if (gettoken(amassd(k, i), ierr2) /= 0) goto 4044
        ...
        if (gettoken(nocont, ierr2) /= 0) goto 4045
        nodac = nodye + nocont
        i = max(nocont, 1)
        allocate   (ascal (i))
        ...
        do i = 1, nocont
            ...
            if (gettoken(nmconr(i), ierr2) /= 0) goto 4046
         ...
         if ( .not. oil  .and. .not. leeway) then
            ...
            if (gettoken(fiwaste(i + nodye), ibuffer, radius(i + nodye), itype, ierr2) /= 0) goto 4043
            if (itype==2) then
            ...
            elseif (itype==3) then
                ...
                if (ierr2 /= 0) go to 1703
                call getdim_dis (lunfil, fiwaste(i + nodye), nrowsmax, lun2)
                close (lunfil)
            endif

            if (gettoken(wparm (i + nodye), ierr2) /= 0) goto 4043
            ...
            do k = 1, nosubs
                if (gettoken(stoch(k, i), ierr2) /= 0) goto 4050
                write (lun2, 2290) substi(k), stoch(k, i)
            enddo

            !       read time points cont's

            if (gettoken(ictmax(i), ierr2) /= 0) goto 4051
            ...
            do k = 1, ictmax(i)
                ...
                if (gettoken(id, ierr2) /= 0) goto 4052
                if (gettoken(ih, ierr2) /= 0) goto 4052
                if (gettoken(im, ierr2) /= 0) goto 4052
                if (gettoken(is, ierr2) /= 0) goto 4052
                if (gettoken(ftime(i, k), ierr2) /= 0) goto 4052
                ictime(i, k) = id * 86400 + ih * 3600 + im * 60 + is
                if (k == 1 .and. ictime(i, k) /= itstrtp) then
                    write (lun2, 2360)
                    ierr = ierr + 1
                endif
            enddo
            ...
            if (ictmax(i) > 0) then
                if (ictime(i, ictmax(i)) /= itstopp) then
                    write (lun2, 2381)
                    ierr = ierr + 1
                endif
            endif
        ...
        if (nocont > 0) deallocate(ascal)

        ! read actual waste polygons
        if (nodac > 0 .and. nrowsmax > 0) then
        ...
        if (gettoken(noudef, ierr2) /= 0) goto 4053
        ...
        do i = 1, noudef
            ...
            if (gettoken(wparm(i + nodac), ierr2) /= 0) goto 4054
            if (gettoken(uscal(i), ierr2) /= 0) goto 4054
            ...
            if (gettoken(id, ierr2) /= 0) goto 4055
            if (gettoken(ih, ierr2) /= 0) goto 4055
            if (gettoken(im, ierr2) /= 0) goto 4055
            if (gettoken(is, ierr2) /= 0) goto 4055
            if (gettoken(isubud(i), ierr2) /= 0) goto 4055
            ...
            !
            if (gettoken(ifopt(i), ierr2) /= 0) goto 4056

            !       read filename delwaq or particle coordinates file

            if (gettoken(finud(i), ierr2) /= 0) goto 4057
            ...
            if (ifopt(i) /= 2) then
                ...
                !
                if (gettoken(id, ierr2) /= 0) goto 4058
                if (gettoken(ih, ierr2) /= 0) goto 4058
                if (gettoken(im, ierr2) /= 0) goto 4058
                if (gettoken(is, ierr2) /= 0) goto 4058
                if (gettoken(nosud(i), ierr2) /= 0) goto 4058
                if (gettoken(isfud(i), ierr2) /= 0) goto 4058
                write (lun2, 2376)
            ...
            if (iftime(i) < itstrtp .or. iftime(i) > itstopp) write (lun2, 2176) i
        ...
        decays = 0.0
        if (gettoken(idtset, ierr2) /= 0) goto 4059
        if (idtset > 0) then
            call alloc ("idtime", idtime, idtset)
            call alloc ("decay ", decay, nosubs, idtset)
            do i = 1, idtset
                if (gettoken(id, ierr2) /= 0) goto 4060
                if (gettoken(ih, ierr2) /= 0) goto 4060
                if (gettoken(im, ierr2) /= 0) goto 4060
                if (gettoken(is, ierr2) /= 0) goto 4060
                do isb = 1, nosubs
                    if (gettoken(decay(isb, i), ierr2) /= 0) goto 4060
                enddo
                idtime(i) = id * 86400 + ih * 3600 + im * 60 + is
                if (i == 1 .and. idtime(i) /= itstrtp) then
                    write (lun2, 2360)
                    ierr = ierr + 1
                endif
                if (i > 1) then
        ...
        nvsfour = 6
        if (gettoken(anfac, ierr2) /= 0) goto 4064
        if (gettoken(irfac, ierr2) /= 0) goto 4065
        irfac = max(1, irfac)
        write (lun2, 2339) anfac, irfac

        !     read time points settling velocities

        if (gettoken(ivtset, ierr2) /= 0) goto 4061
        if (ivtset > 0) then
            ...
            do isb = 1, nosubs
                ...
                write (lun2, 2373)
                do i = 1, ivtset
                    if (gettoken(id, ierr2) /= 0) goto 4062
                    if (gettoken(ih, ierr2) /= 0) goto 4062
                    if (gettoken(im, ierr2) /= 0) goto 4062
                    if (gettoken(is, ierr2) /= 0) goto 4062
                    do k = 1, nvsfour
                        if (gettoken(vsfour(k, isb, i), ierr2) /= 0) goto 4062
                    enddo
                    write (lun2, 2375) id, ih, im, is, (vsfour(k, isb, i), k = 1, nvsfour)
                    if (isb == 1) then
                        ivtime(i) = id * 86400 + ih * 3600 + im * 60 + is
                        if (i == 1 .and. ivtime(i) /= itstrtp) then
                            write (lun2, 2360)
                            ierr = ierr + 1
                        endif
                        if (i > 1) then
        ...
        if (use_settling) then
            if (gettoken(taucs, ierr2) /= 0) goto 4063
            if (gettoken(tauce, ierr2) /= 0) goto 4063
            if (gettoken(chezy, ierr2) /= 0) goto 4063
        ...
        do i = 1, npgrid
            ...
            if (xw1f <= 1.0e-4 .or. yw1f <= 1.0e-4) then
                write (lun2, 2390)
                ierr = ierr + 1
            endif
        enddo
        ...
        if (npgrid > 0) then
            ...
            do i = 1, npgrid
                cwork = "nmcell"
                if (i > 1) write(cwork(7:), '(i3.3)') i
                nullify (pg(i)%nmcell)
                call alloc (cwork, pg(i)%nmcell, pg(i)%nmap, pg(i)%mmap)
                cwork = "amap"
                if (i > 1) write(cwork(5:), '(i3.3)') i
                nullify (pg(i)%amap)
        ...
        2130 format(/, '  Start time map file  :', i4, 'D-', i2.2, &
                'H-', i2.2, 'M-', i2.2, 'S.')
        2131 format(/, '  Start time time-hist.:', i4, 'D-', i2.2, &
                'H-', i2.2, 'M-', i2.2, 'S.')
