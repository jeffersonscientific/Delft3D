...
!
!  Copyright (C) 2005-2025, the Regents of the University of Minnesota
!
...
!      permselect = 3    ! Check if 3 still possible
   ...
   subroutine cgsaad(its, na, nocg, jaini, jabcgstab, ierror, res)
      use m_saadf, only: multic, indset2, dperm, permsimple, permsimpleinverse
      implicit none
...
80    if (rp) then
         call tidycg(n, ipar, fpar, sol, w(3 * n + 1:4 * n))
      else
         call tidycg(n, ipar, fpar, sol, w(4 * n + 1:5 * n))
      end if
!
      return
   end subroutine cg
...
110   if (rp) then
         call tidycg(n, ipar, fpar, sol, wk(:, 4))
      else
         call tidycg(n, ipar, fpar, sol, wk(:, 5))
      end if
      return
   end subroutine cgnr
...
110   if (rp) then
         call tidycg(n, ipar, fpar, sol, w(:, 6))
      else
         call tidycg(n, ipar, fpar, sol, w(:, 7))
      end if
      return
   end subroutine bcg
...
110   if (rp) then
         ...
         call tidycg(n, ipar, fpar, sol, w(:, 8))
      end if
!
      return
...
!     update I
!
      theta = ddot(n, w(:, 3), 1, w(:, 3), 1) / (tao * tao)
      sigma = one / (one + theta)
      ...
      if (ipar(7) == 1) then
         if (ipar(3) == -1) then
            fpar(3) = eta * sqrt(ddot(n, w(:, 9), 1, w(:, 9), 1))
            fpar(4) = fpar(1) * fpar(3) + fpar(2)
...
!     this is an approximation
      ...
      if (ipar(3) == 999) then
         ...
         fpar(6) = eta * sqrt(ddot(n, w(:, 9), 1, w(:, 9), 1))
...
110   if (rp) then
         call tidycg(n, ipar, fpar, sol, w(:, 10))
      else
         call tidycg(n, ipar, fpar, sol, w(:, 11))
      end if
!
      return
...
110   ipar(7) = ipar(7) + 1
      ...
      fpar(11) = fpar(11) + n
!
120   fpar(3) = sqrt(ddot(n, w(:, 3), 1, w(:, 3), 1))
      fpar(11) = fpar(11) + n + n
      fpar(5) = fpar(3)
      fpar(7) = fpar(3)
      zeta = fpar(3)
...
36    if (ipar(3) == 999) then
      ...
      fpar(5) = res * sqrt(ddot(n, w(:, i2 + 2), 1, w(:, i2 + 2), 1))
      fpar(11) = fpar(11) + 2 * n
      if (ipar(3) < 0) then
...
!
      nrm0 = ddot(n, vec(:, ind), 1, vec(:, ind), 1)
      ops = ops + n + n
      thr = nrm0 * reorth
      if (nrm0 <= zero) then
         ierr = -1
...
!
      nrm1 = sqrt(ddot(n, vec(:, ind), 1, vec(:, ind), 1))
      ops = ops + n + n
75    hh(ind) = nrm1
      if (nrm1 <= zero) then
         ierr = -3
