...
subroutine orthonet_smooth_aspect(aspect, iexit)
   ...
   if (ATPF == 1d0) return ! no smoothing

   if (ITAPSM < 1) then ! set aspect=1d0
      aspect = 1d0
      return
   end if
      ...
      do kk = 1, numL
         ...
         do kk1 = 1, nn
            ...
            if (cosphi > COSMIN) then
               ...
               !           set weight ww1 < 0
            else if (abs(cosphi) < COSMAX) then
               nmkx(kk) = nmkx(kk) + 1
               num = nmkx(kk)
