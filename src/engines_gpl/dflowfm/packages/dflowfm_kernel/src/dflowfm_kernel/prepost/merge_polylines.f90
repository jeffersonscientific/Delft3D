...
!        check if polylines end/start points are coinciding
         ipol1 = 0
         ipol2 = 0
         if (dbdistance(xstart1, ystart1, xstart2, ystart2, jsferic, jasfer3D, dmiss) <= dtol) then
            ipol1 = jstart1
            ipol2 = jstart2
         else if (dbdistance(xstart1, ystart1, xend2, yend2, jsferic, jasfer3D, dmiss) <= dtol) then
            ipol1 = jstart1
            ipol2 = jend2
         else if (dbdistance(xend1, yend1, xstart2, ystart2, jsferic, jasfer3D, dmiss) <= dtol) then
            ipol1 = jend1
            ipol2 = jstart2
         else if (dbdistance(xend1, yend1, xend2, yend2, jsferic, jasfer3D, dmiss) <= dtol) then
            ipol1 = jend1
            ipol2 = jend2
         end if
...
!  remove trailing missing values
   if (NPL > 1) then
      do while ((xpl(NPL) == DMISS .or. ypl(NPL) == DMISS) .and. NPL > 1)
         NPL = NPL - 1
      end do
   end if
