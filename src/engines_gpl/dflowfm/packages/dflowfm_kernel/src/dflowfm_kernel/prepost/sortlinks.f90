...
!         if ( kmask(k0).ne.0 ) cycle       ! already visited
      ...
      do k1 = 1, N - 1 ! loop over the links and reorder the remaining links
         ...
         do k2 = k1 + 1, N ! loop over remaining links
            ...
            if (lne(1, L1) == lne(1, L2) .or. lne(1, L1) == lne(i2, L2) .or. &
                lne(i1, L1) == lne(1, L2) .or. lne(i1, L1) == lne(i2, L2)) then
               !              next link found
               knext = k2
               exit
            end if
         ...
         if (k1 == 1 .and. knext == N .and. N > 2) then ! wrong direction
            knext = 0
            L2 = 0
         end if
...
!                  goto 1234
         ...
         if (knext == 0) then ! no next link found
!              error: deactivate node
         ...
         !        swap links
         if (nod(k0)%lin(k1 + 1) /= L2) then ! fold
