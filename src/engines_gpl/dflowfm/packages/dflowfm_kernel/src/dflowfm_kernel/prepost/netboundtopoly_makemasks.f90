...
!  make node and link masks
   ...
   do L = 1, numL
      ...
      if (kc(k1) == -1) then ! mask of node k1 not yet determined
      ...
      if (kc(k1) == 1) then
         if (kc(k2) == -1) then ! mask of node k1 not yet determined
         ...
         if (kc(k2) == 1) then ! both nodes inside selecting polygon
            Lc(L) = 1
         end if
      end if
   end do
