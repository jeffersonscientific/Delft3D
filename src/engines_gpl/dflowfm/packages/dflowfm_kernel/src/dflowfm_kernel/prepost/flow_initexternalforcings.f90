...
integer function flow_initexternalforcings() result(iresult)              ! This is the general hook-up to wind and boundary conditions
 ...
 integer                       :: numnos, numnot, numnon ! < Nr. of unassociated flow links (not opened due to missing z- or u-boundary)
   ...
   allocate ( xbndu(tmp_nbndu), ybndu(tmp_nbndu), idbndq_left(tmp_nbndu), idbndq_right(tmp_nbndu), bndq_wR(tmp_nbndu), bndq_wL(tmp_nbndu), xy2bndu(2,tmp_nbndu), kbndu(n4,tmp_nbndu), kdu(tmp_nbndu) , stat=ierr)
   call aerr('xbndu(tmp_nbndu), ybndu(tmp_nbndu), idbndq_left(tmp_nbndu), idbndq_right(tmp_nbndu), bndq_wR(tmp_nbndu), bndq_wL(tmp_nbndu), xy2bndu(2,tmp_nbndu), kbndu(n4,tmp_nbndu), kdu(tmp_nbndu)', ierr, tmp_nbndu*(n4+9) )
   ...
   allocate ( zbndq_left_t0    (tmp_nbndu) , stat = ierr       )
   call aerr('zbndq_left_t0    (tmp_nbndu)', ierr , tmp_nbndu  )
   allocate ( zbndq_right_t0   (tmp_nbndu) , stat = ierr       )
   call aerr('zbndq_right_t0   (tmp_nbndu)', ierr , tmp_nbndu  )
   allocate ( zbndq_left_t1    (tmp_nbndu) , stat = ierr       )
   call aerr('zbndq_left_t1    (tmp_nbndu)', ierr , tmp_nbndu  )
   allocate ( zbndq_right_t1   (tmp_nbndu) , stat = ierr       )
   call aerr('zbndq_right_t1   (tmp_nbndu)', ierr , tmp_nbndu  )
