...
!        check numbering
         if (jC(ipointdiag) /= irow) then
            call qnerror('fillsystem_advec: numbering error', ' ', ' ')
            goto 1234
         end if

         if (jC(ipointdiag + ishift) /= irow + 1) then
            call qnerror('fillsystem_advec: numbering error', ' ', ' ')
            goto 1234
         end if
...
!           check numbering
            if (jC(ipoint) /= (ln(1, LL) + ln(2, LL) - kk - 1) * 2 + 1) then
               call qnerror('fillsystem_advec: numbering error', ' ', ' ')
               goto 1234
            end if

            if (jC(ipoint + ishift) /= (ln(1, LL) + ln(2, LL) - kk - 1) * 2 + 2) then
               call qnerror('fillsystem_advec: numbering error', ' ', ' ')
            end if
