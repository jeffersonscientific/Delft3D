...
!     Program to decompose a PROCES.ASC file into tables
     ...
     !----------------------------------------------------------------------c
      ...
      if ( duprol ) then
          ...
          read ( io_asc , '(i10)', iostat = ierr ) naanta
          if ( ierr /= 0 ) then
              write(*,*) 'Error reading number of active substances (DUPROL) at line', linecount
              error stop
          endif
          ...
          do iaanta = 1,naanta
              ...
              read ( io_asc , FMT32, iostat = ierr ) c10,c1,c50,c20
              if ( ierr /= 0 ) then
                  write(*,*) 'Error reading active substance (DUPROL) at line', linecount
                  error stop
              endif
              value = -999.
              call upd_p2 ( c10, c50, value, 0, newtab, grp, io_mes, iitem, c20, newfrm, .false. )
          ...
          read ( io_asc , '(i10)', iostat = ierr ) naanta
          if ( ierr /= 0 ) then
              write(*,*) 'Error reading number of inactive substances (DUPROL) at line', linecount
              error stop
          endif
          ...
          do iaanta = 1,naanta
              ...
              read ( io_asc , FMT32, iostat = ierr ) c10,c1,c50,c20
              if ( ierr /= 0 ) then
                  write(*,*) 'Error reading inactive substance (DUPROL) at line', linecount
                  error stop
              endif
              value = -999.
