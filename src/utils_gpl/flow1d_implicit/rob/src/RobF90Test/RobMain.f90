...
PROGRAM RobTest
  ...
  character (len=25)              :: TestSetID = 'TestSet ID'
  ...
  n = FRobQuantityPropertySetValue("q2",Quantity(4),"Property 2","New Property Value")
  ...
  n = FRobElementsetAddIDBasedMultiple(ModelID, TestSetID,'TestSet Description', ElementID(1:3));

  ! Create the ExchangeItems for the Quantity and Quantityset
  do k=1,2
    n = FRobExchangeitemAdd (ModelID, Quantity(k), TestSetID, PROVIDING_ROLE);
    n = FRobExchangeitemAdd (ModelID, Quantity(k), TestSetID, ACCEPTING_ROLE);
  end do

  ! Check RobPutDouble (single Quantity)
  D0(1) = 10.; D1(1) = 20.
  write(fp,'(a)') 'RobPutDouble :'
  n = FRobPutDouble(ModelID, '', Quantity(1),TestSetID, PROVIDING_ROLE, 1, D0 );
  write(fp,'(2a,f8.2)') 'FRobPutDouble (all elements same value) : Quantity '//Quantity(1), ' ElementSetID '//'TestSet ID PROVIDING ',D0
  n = FRobPutDouble(ModelID, '', Quantity(1),TestSetID, ACCEPTING_ROLE, 1, D1 );
  ...
  do j=3,5
    ...
    m = FRobPutDouble(ModelID, Quantity(2), Quantity(j), TestSetID, PROVIDING_ROLE, n, pD0 );
    ...
    m = FRobPutDouble(ModelID, Quantity(2), Quantity(j), TestSetID, ACCEPTING_ROLE, n, pD1 );
  ...
  m = FRobGetDouble(ModelID, '', Quantity(1),TestSetID, PROVIDING_ROLE,n, pD0 );
  write(fp,'(a,i3,a)', advance='no')'RobGetDouble Quantity '//Quantity(1)//' Role = PROVIDING, Quantityset =TestSet ID (result=',m,')'
  write(fp,'(1000f8.2)') (pD0(m),m=1,n)

  m = FRobGetDouble(ModelID, '', Quantity(1),TestSetID, ACCEPTING_ROLE, n, pD1 );
  ...
  do j=3,5
     m = FRobGetDouble(ModelID, Quantity(2), Quantity(j), TestSetID, PROVIDING_ROLE, n, pD0 );
     ...
     m = FRobGetDouble(ModelID, Quantity(2), Quantity(j), TestSetID, ACCEPTING_ROLE, n, pD1 );
