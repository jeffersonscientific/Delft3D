c
c Lezen lozingen/onttrekkingen van uitwisselingsbestand
c
   ...
   10 read (120,'(a)') path
      ...
      close (120)
c     write(juer,'(1x,a40)') fullname

      EntityName = 'NwDwEx'
      n = 0
        
c      write (cTB,'(i4,a,i4.4,a)') TB/10000, '.',
c     &                         TB - (TB/10000 * 10000),
c     &                        '000000'
c      write (cTE,'(i4,a,i4.4,a)') TE/10000, '.',
c     &                         TE - (TE/10000 * 10000),
c     &                        '000000'
     
c      WRITE(FullName,'(3a)') path(1:strlen(path)),
c    &                       EntityName(1:strlen(EntityName)),'D'
c      fullname = 'NwDwEx'
...
c JC 12/4/2000
c
c    Ingelezen waarde vasthouden in qlat(istat,3)
        ...
        r = (r .and. nE .eq. 0)
c JC 12/4/2000: Demand gewijzigd in Alloc (op meerdere plaatsen
        call RDVLD_D (nE, FullName, EntityName, 'DwAlloc', dDwAlloc)
        r = (r .and. nE .eq. 0)

        if (r) n=n+1
c        write (*,*) n      
c        if (r .and. (cTimeBegin .eq. cTB .and. cTimeEnd .eq. cTE)) then
