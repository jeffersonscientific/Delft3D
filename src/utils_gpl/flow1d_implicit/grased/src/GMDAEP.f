      subroutine GMDAEP (igpbou ,ngrid  ,nfrac  ,dtm    ,alphac ,
...
c
c Module:             GMDAEP (Graded Morphology Delta Area for
c                             End Point)
c
...
c History:
c $Log: gmdaep.F,v $
c Revision 1.2  1995/09/27  10:11:28  kuipe_j
c Maintenance
c
c
c***********************************************************************
...
c
c     Calculate point on location x = n - 1/2
c
      igpcel = igpbou-1
c
...
c
      dtm2 = sngl(dtm) * .5
      rdx  = 2. / (x(igpbou) - x(igpcel))
c
      do 10 jf=1,nfrac

c        Calculate lateral integral

         ili = source(igpbou-1,jf) * dtm2
c
c        Calculate delta A
c
         deltaa(igpbou,jf) = ( intbou(jf) - intiph(jf)- ili ) * rdx
