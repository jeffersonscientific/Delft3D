...
!
      IF (KSPHER.GT.0) THEN
        DO IC = 1, ICMAX
          IF ( KCGRD(IC).EQ.1 ) CYCLE   ! if point is not valid, then cy!cle
          COSLAT(IC) =
...
!  1. Updates
!
!     20.44, Sep. 96: Subroutine completely reorganised subroutine has n!ew name
!                     instead of COUNT
...
!                     comments added
!                     minimal value of ISSTOP is 4 (in view of CGSTAB so!lver)
!     40.13, July 01: values of DX2 and DY2 corrected in repeating coord!inates
!     40.30, Mar. 03: introduction distributed-memory approach using MPI!
!     40.41, Sep. 04: part concerning computation of geometric quantitie!s
...
!        40.41, Oct. 04: common blocks replaced by modules, include file!s removed
!
!     2. PURPOSE
!
...
!       CAX(ID,IS,IC)     = 0.   {propagation velocity of energy in X-di!r.}
!       CAY(ID,IS,IC)     = 0.   {propagation velocity of energy in Y-di!r.}
!     ---------------------------------------------------------
!   Else if current is on (ICUR > 0) then,
!     For every point in S and D-direction do,  {using the output of SWA!PAR}
...
!
      ...
      USE M_PARALL                                                      !  40.31
      USE M_DIFFR                                                       !  40.21
      USE SwanIEM, ONLY: ntf, dfiem, sflog                              !  41.85
!
      IMPLICIT NONE
!
!
...
!     40.02, Jan. 00: Introduction limiter dependent on Cx, Cy, Dx and D!y
...
!     40.61, Dec. 06: correction DO loop 60 (IDCMIN, IDCMAX -> IDDLOW,ID!DTOP)
!     41.06, Mar. 09: add option of limitation of velocity in theta-dire!ction
...
!     SPCSIG: Relative frequencies in computational domain in sigma-spac!e
...
!
      ...
      INTEGER  :: IS                        ! Counter of relative freque!ncy band    !   30.80
      ...
      INTEGER  :: KCG2  ,KCG3 , KCG4, KCG5  ! grid addresses of neighbou!ring grid points   !   30.80
      INTEGER  :: IX1                       ! IX1   Counter of gridpoint!s in x-direction  !   40.03
      INTEGER  :: IY1                       ! IY1   Counter of gridpoint!s in y-direction  !   40.03
!
!     local real variables :
!
      REAL  :: KD1                                    ! KD1           wa!venumber * depth
      REAL  :: VLSINH                                 ! VLSINH        si!nh of KD1
      REAL  :: COEF                                   ! COEF          au!x. quantity
      ...
      REAL  :: DCDX   ,DCDY                           ! celerity gradien!t
      ...
      REAL  :: CGLOC1                                 ! group velocity a!t active grid point ! 40.59
      REAL  :: KLOC1, KLOC2, KLOC3, KLOC4, KLOC5      ! wavenumbers at a!ctive and neighbouring grid points ! 40.59
...
!
      REAL  :: ALPHA                                  ! upper limit of C!FL restriction ! 41.35
      ...
      REAL  :: PP                                     ! power of the fre!quency dependent limiter on refraction ! 41.06
!
!  8. Remarks
!
...
!     Experiments suggest that @C/@x method with coarse resolution yield!s results that are similar to those                          40.59
!     using @C/@x or @h/@x with high resolution.                        !                                                             40.59
!     By contrast, @h/@x method with coarse resolution yields considerab!ly different result.                                         40.59
!  Further, using @C/@x allows for adding refraction by additional varia!bles included in dispersion relation.                        40.59
!     Specifically, non-rigid seafloor (mud) can cause refraction even w!hen water layer thickness (i.e. depths) are uniform.         40.59
...
!  Similar to above, the difference is only in numerics; experimental re!sults suggest                                                40.59
...
!  Further still, implementation of the upwind scheme in versions such a!s 40.41 and 40.51 used sweeping to                           40.59
...
!  It would be possible to have separate code for falling back to the up!wind scheme,                                                 40.59
!  but this may require sweeping, which would mean much additional code,! see e.g. code of public release v40.51                      40.59

!  Note: RDX and RDY are unavailable for P5-P2 and P4-P3, so we use as a!pproximation, 0.5*RDX(1), etc.                               40.59
!  Experience with implementation of more precise calculations of RDX,RD!Y, specifically with the SORDUP scheme,                      40.59
!      yielded imperceptible change in results. However, this could be a!dded later if sufficiently motivated.                        40.59

!  The depth refraction limitation procedure (i.e. IREFR=-1) has been re!moved, because it is basically a "dirty fix" which           40.59
!      we hope/expect has been made unnecessary by the other changes her!e. If we are wrong about this, the "dirty fix" can be        40.59
!      restored, but this is not straightforward, since C is computed fr!om k, which is an input argument not affected by IREFR.      40.59
...
!      DHDX = 1.0*RDX(1) * (DLOC1-DLOC2) + 1.0*RDX(2) * (DLOC1-DLOC3) ! !upwind scheme
      DHDX = 0.5*RDX(1) * (DLOC5-DLOC2) + 0.5*RDX(2) * (DLOC4-DLOC3) ! c!entered scheme       40.59
!     *** @D/@Y ***
!      DHDY = 1.0*RDY(1) * (DLOC1-DLOC2) + 1.0*RDY(2) * (DLOC1-DLOC3) ! !upwind scheme
      DHDY = 0.5*RDY(1) * (DLOC5-DLOC2) + 0.5*RDY(2) * (DLOC4-DLOC3) ! c!entered scheme       40.59

      IF ( ICUR .EQ. 1 .AND. IQCM.EQ.0 ) THEN  !           *** current i!s on ***
!
!     *** @Ux/@X ***
...
!       *** coefficients for CAS -> function of IS only ***
        ...
        VLSINH = SINH (2.* KD1 )
        COEF   = SPCSIG(IS) / VLSINH   ! also needed for CAD, if @h/@x m!ethod used
...
!         Intuitively, one may expect that variable currents could be in!cluded via @C/@x. However, this is not the case.
...
!            --- adapt the velocity in case of diffraction              !  40.21
     ...
     &                         SINSIN(ID)*DUYDX -
     &                         COSCOS(ID)*DUXDY ! add currents, Christof!fersen, Tolman, Ris, et al.
...
!
!     References:
!     W. E. Rogers, J. M. Kaihatu, H. A. H. Petit, N. Booij and L. H. Ho!lthuijsen,
...
!
      LOGICAL  ANYBIN(MDC,MSC)
!
!     o  CAD    3D    Wave transport velocity in D-direction, function o!f
!                     (ID,IS,IC)
!     i  CAX    3D    propagation velocity in X-direction (CGO+UX)
!     i  CAY    3D    propagation velocity in Y-direction (CGO+UY)
!     i  YCGRID 2D    Y-coordinate (latitude) for each geographic grid p!oint
!     i  ECOS   1D    Represent the values of Cos(Theta) of each spectra!l
!                     direction
!     i  ESIN   1D    Represent the values of Sin(Theta) of each spectra!l
!                     direction
!
!     Changed ICMAX to MICMAX, since MICMAX doesn't vary over gridpoint !  40.22
      REAL  :: CAD(MDC,MSC,MICMAX)                                      !  40.22
...
!     1. UPDATE
!
!        30.72, Oct. 97: changed floating point comparison to avoid equa!lity
!                        comparisons
...
!        33.09, June 99: commons swcomm2 and swcomm3 introduced, argumen!t list
...
!        ICMAX   int, i     Maximum counter for the points of the molecu!le
!        ISSTOP  int, i     highest spectral frequency counter in the sw!eep
!        IDCMIN  int, i     minimum value of direction counter in this s!weep
!        IDCMAX  int, i     maximum value of direction counter in this s!weep
...
!        IMATRA  rea, i/o   array  Coefficients of right hand side of ma!trix
!        OBREDF  rea, i     action reduction factors, function of freq a!nd
!                           direction
...
!
!         This business of doing rollback regardless of ITERMX was an ar!tifact 40.08
!         and has been removed. Thus, the code reverts to its form in v4!0.01   40.08
!
...
!     0. AUTHORS
!
!        33.10: Nico Booij and Erick Rogers (changes related to the SORD!UP scheme)
!        33.09: Nico Booij (changes related to spherical coordinates)
!        40.08: Erick Rogers
!        40.41: Marcel Zijlema
...
!     1. UPDATE
!
!        33.10, Jan. 2000: subroutine SORDUP created. It is a modified S!TRSXY.
!        40.08, Mar. 2003: Improve scheme to use dx,dy calculated over t!wo grid
!                          spaces (instead of one) where appropriate. Th!is involves
!                          the use of RDX(3),RDX(4),RDY(3),RDY(4). This !may or may not
!                          be a noticeable improvement. (I have not seen! an example of a
!                          case where the original SORDUP does poorly re!lative to BSBT,
!                          so this is a speculative improvement).
!                          Remove option for controllable 1st order diff!usion ("XYMU",
!                          "THETAK", etc.)
!        40.41, Oct. 04: common blocks replaced by modules, include file!s removed
...
!            @x
!
!        [1.5*CAX(ID,IS,1)*AC2(ID,IS,KCGRD(1))-2.0*CAX(ID,IS,2)*AC2(ID,I!S,KCGRD(2))
...
!            @y
!
!        [1.5*CAY(ID,IS,1)*AC2(ID,IS,KCGRD(1))-2.0*CAY(ID,IS,3)*AC2(ID,I!S,KCGRD(3))
!        +0.5*CAY(ID,IS,7)*AC2(ID,IS,KCGRD(7))]/DY
!
!        ADD TO DIAGONAL:
!        +1.5*CAX(ID,IS,1)/DX+1.5*CAY(ID,IS,1)/DY
!        ADD TO RHS:
!        +[2.0*CAX(ID,IS,2)*AC2(ID,IS,KCGRD(2)-0.5*CAX(ID,IS,6)*AC2(ID,I!S,KCGRD(6)]/DX
!        +[2.0*CAY(ID,IS,3)*AC2(ID,IS,KCGRD(3)-0.5*CAY(ID,IS,7)*AC2(ID,I!S,KCGRD(7)]/DY
!
!     4. PARAMETERLIST
!
!        KCGRD   int, i     Point index for grid points in comp molecule!  30.40
...
!     1. UPDATE
!
!        33.08, July 98: SANDL: New subroutine using a Stelling and Leen!derste
...
!
!        computational stencil:                                     40.0!3
!                                                                   33.0!8
!      IY+1                 o 11 o 4  o 12                          33.0!8
!                           |    |    |                             33.0!8
!                 8    6    | 2  | 1  | 5                           33.0!8
!      IY         o----o----o----*----o                             33.0!8
!                           |    |    |                             33.0!8
...
!
!         Notes (Rogers, Jan 10 2013) : I noticed a few years ago that t!here is
!            some asymmetry to this anti-GSE implementation that can be !removed.
!            In some tests, the asymmetry results in something that look!s like a lima bean
!            where we would expect an ellipsoid.
!            If we do this, it may make some swell dispersion look a bit! more "natural"
!            Good news: the change is to the variable "D12AC", which mea!ns that it is not complicated by RDX RDY
...
!                        thus indices are :   + IND12              -IND1!1              -IND13            +IND10
!
!        The finite difference scheme for @^2/@x^2 is created by taking !this relation:
!          @A/@x=(A(i+0.5)-A(i-0.5))/dx
!        ...and applying it twice, @(@A/@x)/@x=(A(i+1)-2*A(i)+A(i-1))/(d!x^2)
!
!        The OLD finite difference scheme for @(@A/@x)/@y is mentioned i!n our paper, Rogers et al. (2002)
...
!        CAX     rea, i     3D array    propagation velocity in x  new t!ime level
!        CAY     rea, i     3D array    propagation velocity in y
!        CAX1    rea, i     3D array    propagation velocity in x  old t!ime level
...
!         fastest wave.                                                 !  40.08
      IF(MYU.GT.10.0.AND..NOT.NOWARN)THEN                               !  40.08
...
!
!         To avoid violation of the ANSI standard this statement is spli!t 40.02
!
            FXY2 =                                                      !  40.02
...
!  4. Argument variables
!
!     SPCSIG: Relative frequencies in computational domain in sigma-spac!e 30.72
!
      REAL    SPCSIG(MSC)                                               !  30.72
!
!        IC          Dummy variable: ICode gridpoint:
...
!             fully upwind approximation at the boundary of the directio!nal
!             sector
              DIAG12 = - PCD1 - PCD2
              LEAKC1(ID,IS) = -C2
            ELSE
...
!                        direction of wave propagation in absence of cur!rents
!        DEP       2D    Depth as function of X and Y at certain time
!        KWAVE     2D    wavenumber as function of the relative frequenc!y S
!
!     5. SUBROUTINES CALLING
!
!        SWOMPU
...
!
     ...
     &                   GSXWND     ,GENRXY     ,                       !  40.85
     &                   RSXQUA     ,RSXTRI     ,                       !  40.85
     &                   RSXBRA     ,RSXSQC     ,REDSXY     ,           !  41.90 41.80 40.85
     &                   TSXGEO     ,TSXSPT     ,                       !  40.85
     &                   TSXSPS     ,TRANXY     ,                       !  40.85
...
!     30.72, Feb. 98: Introduced generic names XCGRID, YCGRID and SPCSIG! for SWAN
!     40.61, Sep. 06: introduction of all separate dissipation coefficie!nts
!     40.67, Jun. 07: more accurate computation of dissipation terms
!     40.85, Aug. 08: add also propagation, generation and redistributio!n terms
!                     and radiation stress
!     41.75, Jan. 19: adding sea ice
!
!  2. Purpose
!
!     Adds propagation, generation, dissipation, redistribution, leak an!d radiation stress terms
!
!  3. Method
!
!     ---
...
!     surf- and whitecapping dissipation, respectively
!
!     Note on different source terms and transport terms for output purp!osing:
!     these terms in absolute value are integrated over the spectral spa!ce for
!     each grid point. In this way we can estimate the associated time s!cale.
!     Besides these terms we may also compute energy transfer between wa!ves
...
!
 ...
 100  CONTINUE
!
      DSXWCP(KCGRD(1)) = DSXWCP(KCGRD(1)) + ADISSIP(1)     ! whitecappin!g         40.67
      DSXSRF(KCGRD(1)) = DSXSRF(KCGRD(1)) + ADISSIP(2)     ! surf break !          40.67
      DSXBOT(KCGRD(1)) = DSXBOT(KCGRD(1)) + ADISSIP(3)     ! bottom fric!          40.67
      DSXSWL(KCGRD(1)) = DSXSWL(KCGRD(1)) + ADISSIP(4)     ! swell dissi!p         40.88
...
!
      DISSXY(KCGRD(1)) = DISSXY(KCGRD(1)) + SUM(ADISSIP)   ! total dissi!p         40.67
!
      GSXWND(KCGRD(1)) = GSXWND(KCGRD(1)) + AGENERT(1)     ! wind input !          40.85
      GENRXY(KCGRD(1)) = GENRXY(KCGRD(1)) + SUM(AGENERT)   ! total gener!ation     40.85
!
      RSXQUA(KCGRD(1)) = RSXQUA(KCGRD(1)) + AREDIST(1)     ! quadruplets!          40.85
      RSXTRI(KCGRD(1)) = RSXTRI(KCGRD(1)) + AREDIST(2)     ! triads     !          40.85
      RSXBRA(KCGRD(1)) = RSXBRA(KCGRD(1)) + AREDIST(3)     ! Bragg scatt!ering     41.80
      RSXSQC(KCGRD(1)) = RSXSQC(KCGRD(1)) + AREDIST(4)     ! QC scatteri!ng        41.90
      REDSXY(KCGRD(1)) = REDSXY(KCGRD(1)) + SUM(AREDIST)   ! total redis!tribution 40.85
!
      TSXGEO(KCGRD(1)) = TSXGEO(KCGRD(1)) + ATRANSP(1)     ! xy-propagat!ion       40.85
      TSXSPT(KCGRD(1)) = TSXSPT(KCGRD(1)) + ATRANSP(2)     ! theta-propa!gation    40.85
      TSXSPS(KCGRD(1)) = TSXSPS(KCGRD(1)) + ATRANSP(3)     ! sigma-propa!gation    40.85
      TRANXY(KCGRD(1)) = TRANXY(KCGRD(1)) + SUM(ATRANSP)   ! total propa!gation    40.85
!
!       energy transfer between waves and currents due to radiation stre!ss, see page 439 of
...
!     --- compute Laplacian of SQRT(energy) in each computational grid p!oint
