...
!     SWREPS: Reading and processing of the commands defining output point!s
...
!
     ...
     &                   ' 1D-computation')                               !32.02
          GOTO 800                                                        !32.02
        ELSE                                                              !32.02
!         ver 30.20: names of input variables changed, order of data chang!ed
...
!     30.81, Nov. 98: Replaced variable STATUS by IERR (because STATUS is !a
!                     reserved word)
!     30.81, Jan. 99: Replaced variable TO by TO_ (because TO is a reserve!d
!                     word)
!     34.01, Feb. 99: Introducing STPNOW
...
!                     not any more in array containing output request para!meters
...
!     41.72, Nov. 19: user defined number of swell partitions instead of h!ardcoded 9
!
!  2. Purpose
!
...
!
        ...
        ORQTMP%OQR(2) = -1.                                               !40.31
        ORQTMP%RQTYPE = RTYPE                                             !40.31
!       unit reference number NREF is 0, will be determined in output modu!le
...
!     34.01, Feb. 99: Introducing STPNOW
!     30.81, Apr. 99: Prevent negative powers for cosine directional sprea!ding (DSPR);
!                     prevented DSPR > 360 and DSPR < 0 (except for except!ion value).
...
!     40.31, Nov. 03: removing POOL-mechanism, reconsideration of this sub!routine
!     40.41, Oct. 04: common blocks replaced by modules, include files rem!oved
...
!METIS!
!METIS!           next, gather the lists of boundary points to all process!es    43.01
!METIS!
...
!
         ELSE                                                             !41.14
!
!           generate output curves BOUNDARY and BOUND_** for structured gr!ids
!
...
!       else (default case)
!          DONALL is FALSE  and boundary is close and interpolation betwee!n
...
!CJK:03-03-2004:Only a warning must be given, swan must not stop on keywor!d side
!CJK request of Arjen L., so error code is reduced to 1
             CALL MSGERR(1,                                               !40.80 40.31
...
!                        take DX and DY into account when determining dire!ction
...
!
!
!       This subroutine reads the heading of the file to determine locatio!ns
...
!       skip heading lines starting with comment sign ($ as in command fil!e)
...
!             if second quantity is 'NDIR' transform from Nautical to Cart!esian dir.
...
!                     expression for RBSDIR (directions of boundary spectr!um)
!     40.03, Aug. 00: correction WAM nest with spherical SWAN
!     40.13, May  01: order of boundary points in WAM nesting file differe!d
...
!     RL2       length **2 of vector connecting two boundary locations
      ...
      DOUBLE PRECISION :: DXWAM, DYWAM   ! spatial step sizes in nesting f!ile   40.13
      DOUBLE PRECISION :: DXTEST, DYTEST ! distance between two nesting po!ints  40.13
...
!
      LOGICAL   :: DONALL
!
!     DUM_A     real number used for reading a file but not used in any ca!lculation
!     XLON      longitude
!     XLAT      latitude
!     XP2       problem coordinate of a boundary location
!     YP2       problem coordinate of a boundary location
...
!       15. number of heading lines per time step
!       16. number of heading lines per spectrum
!       17. =1: energy dens., =2: variance density, =3 variance energy den!sity (k)
...
!
!     If  DONALL = .TRUE. boundary data correspond to an open boundary oth!erwise
...
!
     ...
     &                    DUM_A, DUM_A, DUM_A
        ENDIF
!       Pass over the lines where the energy spectra is written in the bou!ndary file.
!       The energy spectra is going to be read later, in the subroutine RE!SPEC
!
        IF (BTYPE.EQ.'WW3F') THEN
...
!
     ...
     &    XLAT, IERR
        ENDIF
!
!       in case of nesting coordinates on file are used to determine inter!polation
!       coefficients
!
        IF (KSPHER.EQ.0) THEN
!
!       if SWAN uses Cartesian coordinates, then transform the spherical c!oordinates
...
!  3. Method
!
!     calculate interpolation coefficients between the nesting boundary gr!id and
!     the SWAN computational grid
!
!  4. Argument variables
!
...
!
      ...
      INTEGER, INTENT(IN)     ::  KGRBND(*)        ! array of boundary gri!d points
      INTEGER, INTENT(IN)     ::  XYTST(*)         ! array of (ix,iy) of t!est points
!
      INTEGER, INTENT(IN)     ::  IBOUNC,NBOUNC
!
      REAL                    ::  XP2, YP2         ! coordinates of a poin!t in spectral file   41.14
      REAL, INTENT(IN)        ::  XCGRID(MXC,MYC), YCGRID(MXC,MYC)  ! coor!dinates of computational grid points
!
!     DONALL : logic arguments declare if the nesting boundary is open or !close
!              it is defined by the users
!
...
!     W2        is relative length of projection on line
!     IIPT1     counter checking the grid points related to the nesting bo!undary
!
...
!        first point found in a spectral input file                       !41.14
         ...
         IIPT1 = 1                                                        !40.41
      ELSE
!        RX, RY difference vector between consecutive points of spectral f!ile  41.14
         IF (KSPHER.EQ.0) THEN
            RX  = XP2 - XP1
         ELSE
...
!
!          loop over boundary of computational grid, select boundary point!s
...
!                   next item indicates type of boundary condition
           ...
           ELSE
!
!           KGRBND grid addresses on boundary points, NGRBND number of gri!d points
...
!  2. PURPOSE
!
!       determine whether a grid point is a point where a boundary conditi!on
!       can be applied
!
!  3. METHOD
!
