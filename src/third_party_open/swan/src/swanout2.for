...
!     41.62, Nov. 15: included wave partitioning output (raw partition fil!e)
!
!  2. PURPOSE
!
...
!
      ...
      INTEGER, DIMENSION(*)         :: VOQR   ! pointer in list of variabl!es

      REAL*8 , DIMENSION(2)         :: OQR    ! real coefficients (e.g. ti!me and
                                              ! time step)
...
!
!              --- compute magnitude and direction of wind and ambient cur!rent
!
...
!            Mar. 01: XOFFS and YOFFS were incorrectly added to coordinate!s
...
!     each entry in this routine when the table is written to the PRINT fi!le
!
...
!           For time 18 characters are needed
          ...
          ELSE
            ...
            ELSE
              FSTR = '(F13.X)'
              LFIELD = 13
!             NUMDEC is number of decimals in the table for each output qu!antity
...
!     40.03, May  00: correct time coding option written to heading of fil!e
!            Oct. 00: write 'LOCATION' in upper case
!     40.13, Mar. 01: format for writing coordinates different for Cartesi!an
...
!
!     RTYPE   ch*4   input    type of output request: 'SPEC' for 2-D spect!ral
!                             output, 'SPE1' for 1-D freq. spectrum
!     MIP     int    input    number of output points in set PSNAME
!     ACLOC   real   local    case SPEC: 2-D spectrum at one output locati!on
...
!                     interpolation changed: if a corner of the mesh is dr!y,
!                     exception values are written
!                     argument DEP2 added
!     30.82, Apr. 99: Conversion from m^2/rad/s to m^2/Hz correctly implem!ented
...
!       ACLOC   real   local    |OTYPE|=2: 2-D spectrum at one output loca!tion
...
!       OFAC    real   input    output factor (if INRHOG=1, equal to Rho*G!rav)
!
...
!MatL4!     40.41, Oct. 04: common blocks replaced by modules, include fil!es removed
!MatL4!
!MatL4!  2. Purpose
!MatL4!
...
!MatL4!        According to the IEEE standard, on a 32-bit machine, the re!al
!MatL4!        format has an 8-bit biased exponent (=actual exponent incre!ased
!MatL4!        by bias=127) and a 23-bit fraction or mantissa. The leftmos!t
!MatL4!        bit is the sign bit. Let a fraction, biased exponent and si!gn
...
!MatL4!                   0: double precision (64 bit) floating point numb!ers
!MatL4!                   1: single precision (32 bit) floating point numb!ers
...
!MatL4!     write matrix
!MatL4!
!MatL4!     if necessary, give message that error occurred while writing f!ile
!MatL4!
...
!MatL4      END DO
!MatL4
!MatL4!     --- if necessary, give message that error occurred while writi!ng file
!MatL4
...
!     --- if necessary, give message that error occurred while writing fil!e
