...
#define decode(ptr, otri)                                                     \
  (otri).orient = (int) ((ptrdiff_t) (ptr) & (ptrdiff_t) 3l);         \
  (otri).tri = (triangle *)                                                   \
                  ((ptrdiff_t) (ptr) ^ (ptrdiff_t) (otri).orient)
...
#define encode(otri)                                                          \
  (triangle) ((ptrdiff_t) (otri).tri | (ptrdiff_t) (otri).orient)
...
#define infect(otri)                                                          \
  (otri).tri[6] = (triangle)                                                  \
                    ((ptrdiff_t) (otri).tri[6] | (ptrdiff_t) 2l)
...
#define infected(otri)                                                        \
  (((ptrdiff_t) (otri).tri[6] & (ptrdiff_t) 2l) != 0l)
...
#define sdecode(sptr, osub)                                                   \
  (osub).ssorient = (int) ((ptrdiff_t) (sptr) & (ptrdiff_t) 1l);      \
  (osub).ss = (subseg *)                                                      \
              ((ptrdiff_t) (sptr) & ~ (ptrdiff_t) 3l)
...
#define sencode(osub)                                                         \
  (subseg) ((ptrdiff_t) (osub).ss | (ptrdiff_t) (osub).ssorient)
...

{
  ...
  alignptr = (ptrdiff_t)(pool->nowblock + (ptrdiff_t) 1);
  /* Align the item on an `alignbytes'-byte boundary. */
  pool->nextitem = (VOID *) (alignptr + (ptrdiff_t) pool->alignbytes 
											    - (alignptr % (ptrdiff_t) pool->alignbytes));
  ...
  if (pool->deaditemstack != (VOID *) NULL) {
    ...
    if (pool->unallocateditems == 0)
	{
      ...
      if (*(pool->nowblock) == (VOID *) NULL) 
	  {
		...
		newblock = (VOID **) trimalloc(pool->itemsperblock * pool->itembytes + (int) sizeof(VOID *) + pool->alignbytes);
    ...
    pool->nextitem = (VOID *) ((char *) pool->nextitem + (ptrdiff_t) pool->itembytes);
...

{
  ...
  alignptr = (ptrdiff_t)(pool->pathblock + (ptrdiff_t) 1);
  /* Align with item on an `alignbytes'-byte boundary. */
  pool->pathitem = (VOID *) (alignptr + (ptrdiff_t) pool->alignbytes 
												- (alignptr % (ptrdiff_t) pool->alignbytes));
...

{
  ...
  alignptr = (ptrdiff_t) m->dummytribase;
  m->dummytri = (triangle *)(alignptr + (ptrdiff_t) m->triangles.alignbytes
									  - (alignptr % (ptrdiff_t) m->triangles.alignbytes));
  ...
  if (b->usesegments) {
    ...
    alignptr = (ptrdiff_t)(m->dummysubbase);
    m->dummysub = (subseg *)(alignptr + (ptrdiff_t) m->subsegs.alignbytes
									  - (alignptr % (ptrdiff_t) m->subsegs.alignbytes));
...

{
  ...
  alignptr = (ptrdiff_t)(getblock + (ptrdiff_t) 1);
  foundvertex = (char *) (alignptr + (ptrdiff_t) m->vertices.alignbytes
											 - (alignptr % (ptrdiff_t) m->vertices.alignbytes));
...
#else /* not ANSI_DECLARATORS */
ptrdiff_t randomnation(choices)
unsigned int choices;
#endif /* not ANSI_DECLARATORS */

{
  randomseed = (randomseed * (ptrdiff_t)1366 + (ptrdiff_t)150889) % (ptrdiff_t)714025;
  return randomseed / ((unsigned int)714025 / choices + 1);
}
...

{
  ...
  while (totalsamplesleft > 0) {
    ...
    alignptr = (ptrdiff_t)(sampleblock + (ptrdiff_t) 1);
    firsttri = (char *) (alignptr + (ptrdiff_t) m->triangles.alignbytes
								  - (alignptr % (ptrdiff_t) m->triangles.alignbytes));
