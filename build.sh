...
function DoCMake () {
    ...
    cd    $root/build_$1$2
    echo "cmake . -G "$generator" -B "." -D CONFIGURATION_TYPE="$1" -D CMAKE_BUILD_TYPE=${buildtype}"
          cmake . -G "$generator" -B "." -D CONFIGURATION_TYPE="$1" -D CMAKE_BUILD_TYPE=${buildtype} -D CMAKE_INSTALL_PREFIX=../build_$1$2/install/
    if [ $? -ne 0 ]; then
